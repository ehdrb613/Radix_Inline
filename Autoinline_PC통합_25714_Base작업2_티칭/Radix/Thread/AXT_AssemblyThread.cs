...
namespace Radix
{
    class AXT_AssemblyThread
    {
        ...
        bool AXT_LoadPart_Robot_Get_2 = false; //로봇 로딩에서 제품을 가지러 가는 단계
        bool AXT_LoadPart_Robot_Get_3 = false; //로봇 로딩에서 제품을 가지러 가는 단계
        ...
        bool AXT_WorkPart_Robot_Put_2 = false; //로봇 작업공간에 제품을 놓는 단계
        bool AXT_WorkPart_Robot_Put_3 = false; //로봇 작업공간에 제품을 놓는 단계



        bool AXT_Assembly_BottomReady = false;//Bottom 공급 완료
        bool AXT_Assembly_TopReady = false;//Top 공급 완료
        bool AXT_Assembly_Press = false;//조립 작업
                    ...
                    GlobalVar.SystemStatus != enumSystemStatus.ErrorStop) // 에러상태 아닐 때
                {
                    ...
                    if (AXT_WorkPart_Robot_Put)
                    {
                        if (GlobalVar.Working_Part_Put1_Finish)//쿠카 쓰레드에서 동작이 완료 되면
                        {
                            ...
                            DIO.WriteDOData(enumDONames.Y01_2_Label_Product_Guide_Cylinder, true);
                            DIO.WriteDOData(enumDONames.Y01_3_Label_Pickup_Cylinder, true);

                            AXT_WorkPart_Robot_Put = false;

                            AXT_LoadPart_Robot_Get_2 = true;
                        }
                        else
                        {
                    ...
                    #endregion
                    #region Robot Loading Get
                    if (AXT_LoadPart_Robot_Get_2)
                    {
                        if (GlobalVar.Loading_Part_Get2_Finish)//쿠카 쓰레드에서 동작이 완료 되면
                        {
                            GlobalVar.Loading_Part_Get2 = false;
                            GlobalVar.Loading_Part_Get2_Finish = false;

                            AXT_LoadPart_Robot_Get_2 = false;

                            AXT_WorkPart_Robot_Put_2 = true;
                        }
                        else
                        {
                            GlobalVar.Loading_Part_Get2 = true;//쿠카 쓰레드에서 해당 동작을 함.
                        }
                    }
                    #endregion
                    #region Robot Work Put
                    if (AXT_WorkPart_Robot_Put_2)
                    {
                        if (GlobalVar.Working_Part_Put2_Finish)//쿠카 쓰레드에서 동작이 완료 되면
                        {
                            GlobalVar.Working_Part_Put2 = false;
                            GlobalVar.Working_Part_Put2_Finish = false;

                            AXT_WorkPart_Robot_Put_2 = false;

                            AXT_LoadPart_Robot_Get_3 = true;
                        }
                        else
                        {
                            GlobalVar.Working_Part_Put2 = true;//쿠카 쓰레드에서 해당 동작을 함.
                        }
                    }
                    #endregion
                    #region Robot Loading Get
                    if (AXT_LoadPart_Robot_Get_3)
                    {
                        if (GlobalVar.Loading_Part_Get3_Finish)//쿠카 쓰레드에서 동작이 완료 되면
                        {
                            GlobalVar.Loading_Part_Get3 = false;
                            GlobalVar.Loading_Part_Get3_Finish = false;

                            AXT_LoadPart_Robot_Get_3 = false;

                            AXT_WorkPart_Robot_Put_3 = true;
                        }
                        else
                        {
                            GlobalVar.Loading_Part_Get3 = true;//쿠카 쓰레드에서 해당 동작을 함.
                        }
                    }
                    #endregion
                    #region Robot Work Put
                    if (AXT_WorkPart_Robot_Put_3)
                    {
                        if (GlobalVar.Working_Part_Put3_Finish)//쿠카 쓰레드에서 동작이 완료 되면
                        {
                            GlobalVar.Working_Part_Put3 = false;
                            GlobalVar.Working_Part_Put3_Finish = false;

                            GlobalVar.AXT_LoadPart_Product--;
                            AXT_WorkPart_Robot_Put_3 = false;

                            AXT_Assembly_TopReady = true;
                        }
                        else
                        {
                            GlobalVar.Working_Part_Put3 = true;//쿠카 쓰레드에서 해당 동작을 함.
                        }
                    }
                    #endregion
                    ...
                    {
                        #region 로봇에서 조립 제품이 공급 되었다.
                        if (AXT_Assembly_TopReady)// 로봇 공급 완료 신호 후 동작
                        {
                            #region 조립 실린더가 다운이면
                            ...
                            #endregion
                            #region 조립 실린더가 상승이면
                            else if (DIO.GetDIData(enumDINames.X01_6_Label_Product_Guide_Up_Sensor))
                            {
                                #region 무브 실린더가 전진이면
                                if (DIO.GetDIData(enumDINames.X01_4_Label_Guide_Up_Sensor))
                                {
                                    DIO.WriteDOData(enumDONames.Y01_6_Label_Move_Right, true);
                                    AXT_Assembly_TopReady = false;

                                    Thread.Sleep(500);
                                    AXT_Assembly_Press = true;// 조립 작업 시작
                                }
                                #endregion
                                #region 무브 실린더가 후진이면
                                else if (DIO.GetDIData(enumDINames.X01_5_Label_Guide_Down_Sensor))
                                {
                                    ...
                                    DIO.WriteDOData(enumDONames.Y01_5_Label_Move_Left, false);
                                }
                                #endregion
                            }
                            #endregion
                        ...
                        if (AXT_Assembly_Press)
                        {
                            ...
                            if (DIO.GetDIData(enumDINames.X01_7_Label_Product_Guide_Down_Sensor))
                            {
                                if (!GlobalVar.DryRun)
                                {
                                    ...
                                    //로드셀 데이터 비교
                                    if (GlobalVar.LoadcellData > 25)
                                    {
                                        GlobalVar.LoadcellDataOK = true;
                                    }
                                    else
                                    {
                                        GlobalVar.LoadcellDataOK = false;
                                        //로드셀 데이터 에러
                                    }
                                    ...
                                    if (GlobalVar.UseSecs)
                                    {
                                        ...
                                        GlobalVar.Secs.SetSECS_ProcessData("Loadcell_Data", GlobalVar.LoadcellData.ToString());
        ...
        public bool Cylinder_Ini()
        {
            ...
            if (DIO.GetDIData(enumDINames.X01_0_))
            {
                ...
                else if (DIO.GetDIData(enumDINames.X01_6_Label_Product_Guide_Up_Sensor))
                {
                    ...
                    {
                        DIO.WriteDOData(enumDONames.Y01_2_Label_Product_Guide_Cylinder, false);// 고정을 풀어준다.
                        DIO.WriteDOData(enumDONames.Y01_3_Label_Pickup_Cylinder, false);// 고정을 풀어준다.
