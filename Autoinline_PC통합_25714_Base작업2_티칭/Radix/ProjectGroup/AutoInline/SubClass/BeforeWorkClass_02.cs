                ...
                !ClassDisposing)
            {
                try
                {
                            ...
                            if (GlobalVar.SystemStatus >= enumSystemStatus.AutoRun)
                            {
                                ...
                                if (!StepFinish)
                                {
                                    ...
                                    if (!trayWorkReady &&
                                        ...
                                        ((SNUC_SandingClass)GlobalVar.ProjectClass).beforeLift01.Action == BeforeLiftClass.enumAction.InPutCnyRun)  //투입 단계일때
                                    {
                    ...
                    if (GlobalVar.SystemStatus >= enumSystemStatus.AutoRun)
                    {
                        ...
                        {
                            FuncInline.AddError(FuncInline.enumErrorPart.BeforeWork,
                                FuncInline.enumErrorCode.TimeOut_Error,
                                "샌딩 전 작업 위치 부분 타임아웃 에러가 발생했습니다.\n" +
