...
namespace Radix
{
    public class FuncInlineMove
    {
         ...
         * @param axis 축이름
         */
        public static enumAxis GetPMCAxis(FuncInline.enumLetsAxis axis) // PMC Motion 컨트롤러에 연결된 축 이름으로 x/y 확인
        {
            int mod = (int)axis % 2;
            if (mod == 0)
            {
         ...
         */
        public static double CalcWidthPos(FuncInline.enumLetsAxis axis, double pos) // 일반 폭조절 모터 자체의 거리 계산값을 기구상의 실제 거리로 환산 - 위치값 확인할 때
        {
            switch (axis)
            {
                case FuncInline.enumLetsAxis.ST00_InShuttle_Width:
                    return 300 - pos;
                case FuncInline.enumLetsAxis.ST01_OutShuttle_Width:
                    return 300 - pos;
                case FuncInline.enumLetsAxis.ST02_OutConveyor_Width:
                    return 300 - pos;
                case FuncInline.enumLetsAxis.ST03_InConveyor_Width:
                    return 300 - pos;
                case FuncInline.enumLetsAxis.ST04_NGBuffer:
                    return 300 - pos;
                default:
                    return pos;
            }
         ...
         * @return void
         */
        public static bool MoveAbsolute(uint axis, double Pos) // MoveAbsolute(축번호, 좌표, 속도) 서보 절대위치 이동  
        {
            return MoveAbsolute((int)axis, Pos,FuncInline.LiftSpeed, FuncInline.LiftSpeed * 10, FuncInline.LiftSpeed * 10, FuncInline.LiftSpeed * 100, false, 0);
        }
        /**
         * @brief 서보 절대위치 이동 
         *      지령은 실제 거리 또는 속도값으로 실행하고
         ...
         * @return void
         */
        public static bool MoveAbsolute(uint axis, double Pos, double Vel) // MoveAbsolute(축번호, 좌표, 속도) 서보 절대위치 이동  
        {
            return MoveAbsolute((int)axis, Pos, Vel, Vel * 10, Vel * 10, Vel * 100, false, 0);
        }
         ...
         * @return void
         */
        public static bool MoveAbsolute(int axis, double Pos, double Vel, double Acc, double Dec, double Jerk, bool WaitDone, uint Timeout) // MoveAbsolute(축번호, 좌표, 속도, 가속, 감속, 저크, 완료대기, 대기시간) 서보 절대위치 이동  
        {
            ...
            if (GlobalVar.Simulation)
            {
                ...
                FuncLog.WriteLog("MoveAbsoute : " + axis.ToString() + " - " + Pos.ToString());
                //*
            ...
            double acc = GlobalVar.SystemStatus != enumSystemStatus.AutoRun ? vel * 10 : FuncMotion.GetRealSpeed((int)axis, FuncInline.LiftAccDec);
            double dec = GlobalVar.SystemStatus != enumSystemStatus.AutoRun ? vel * 10 : FuncMotion.GetRealSpeed((int)axis, FuncInline.LiftAccDec);
            //double acc = Acc == 0 ? vel * 10 : FuncMotion.GetRealSpeed((int)axis, Acc);
            //double dec = Dec == 0 ? vel * 10 : FuncMotion.GetRealSpeed((int)axis, Dec);
            double jerk = GlobalVar.SystemStatus != enumSystemStatus.AutoRun ? vel * 50 : FuncMotion.GetRealSpeed((int)axis, FuncInline.LiftAccDec) * 100;



            FuncMotion.MoveAbsolute((uint)axis, pos, vel, acc, dec, jerk);
            //FuncMotion.MoveAbsolute((uint)axis,Pos,Vel);
            return true;
        }
         ...
         * @return void
         */
        public static bool MoveRelative(uint axis, double Pos, double Vel) // MoveAbsolute(축번호, 좌표, 속도) 서보 절대위치 이동  
        {
            return MoveRelative((int)axis, Pos, Vel, Vel * 10, Vel * 10, Vel * 100);
        }
        /**
         * @brief 서보 상대위치 이동 
         *      지령은 실제 거리 또는 속도값으로 실행하고
         ...
         * @return void
         */
        public static bool MoveRelative(int axis, double Pos, double Vel, double Acc, double Dec, double Jerk) // MoveAbsolute(축번호, 좌표, 속도, 가속, 감속, 저크, 완료대기, 대기시간) 서보 절대위치 이동  
        {
            ...
            FuncMotion.MoveRelative((uint)axis, pos, vel, acc, dec, jerk);
            return true;
        }
        ...
        public static void MoveVelocity(uint axis, double Vel, double Acc, double Dec, double Jerk) // MoveVelocity(축번호, 속도, 가속, 감속, 저크) 서보 속도모드 구동 
        {
            ...
            FuncMotion.MoveVelocity(axis,  Vel); ;
        ...
        public static bool MoveLift(FuncInline.enumLiftName lift, FuncInline.enumLiftPos pos, double speed) // 투입리프트 동작. 지정속도로
        {
            try
            {
                ...
                if (GlobalVar.AxisStatus[axis].StandStill &&
                     FuncInlineMove.CheckNearLiftPos(lift, pos))
                {
                    return true;
                }
        ...
        public static FuncInline.enumLiftPos GetLiftPos(int rack)
        {
            ...
            switch (rack)
            {
                case 0:
                    ...
                    if (CheckNearLiftPos(lift, FuncInline.enumLiftPos.FrontPassLine))
                    {
                        return FuncInline.enumLiftPos.FrontPassLine;
                    }
                    if (CheckNearLiftPos(lift, FuncInline.enumLiftPos.FrontScanPos))
                    {
                ...
                case 1:
                    ...
                    if (CheckNearLiftPos(lift, FuncInline.enumLiftPos.RearPassLine))
                    {
                        return FuncInline.enumLiftPos.RearPassLine;
                    }
                    if (CheckNearLiftPos(lift, FuncInline.enumLiftPos.RearNGLine))
                    {
                        return FuncInline.enumLiftPos.RearNGLine;
                    }
                    if (CheckNearLiftPos(lift, FuncInline.enumLiftPos.RearScanPos))
                    {
        ...
        // 리프트 지정 위치 인가?
        public static bool CheckNearLiftPos(FuncInline.enumLiftName lift, FuncInline.enumLiftPos pos)
        {
            FuncInline.enumServoAxis axis = 0;
            if (lift == 0)
            {
                 axis = FuncInline.enumServoAxis.SV02_Lift1;
            }
            else
            {
                axis = FuncInline.enumServoAxis.SV04_Lift2;
            }
        ...
        // 셔틀 지정 위치 인가?
        public static bool CheckNearShuttlePos(FuncInline.enumShuttleName shuttle, FuncInline.enumShuttlePos pos)
        {
            FuncInline.enumServoAxis axis = 0;
            if (shuttle == 0)
            {
                axis = FuncInline.enumServoAxis.SV00_In_Shuttle;
            }
            else
            {
                axis = FuncInline.enumServoAxis.SV01_Out_Shuttle;
            }
            return GlobalVar.AxisStatus[(int)axis].StandStill &&
                    FuncMotion.CheckNearPos(GlobalVar.AxisStatus[(int)axis].Position,
                                     FuncInline.ShuttlePos[(int)shuttle, (int)pos],
                                     0.1);
        }



        public static bool CheckNearPMCWidth(FuncInline.enumLetsAxis axis) // 폭조절 스텝모터가 해당 폭 위치에 있는가?
        {
            double teachingWidth = FuncInline.TeachingWidth[(int)axis];
            double actualWidth = GlobalVar.LetsAxisStatus[(int)axis].Position;
        ...
        // 오토닉스 스탭모터 위치 판단
        public static bool CheckLetsNearPos(FuncInline.enumLetsAxis axis, bool clamp)
        {
            return GlobalVar.LetsAxisStatus[(int)axis].StandStill &&
                    FuncMotion.CheckNearPos(GlobalVar.LetsAxisStatus[(int)axis].Position,
        ...
        public static bool CheckNearPos(FuncInline.enumServoAxis axis, bool clamp)
        {
            return GlobalVar.AxisStatus[(int)axis].StandStill &&
                    FuncMotion.CheckNearPos(GlobalVar.AxisStatus[(int)axis].Position,
                                     FuncInline.TeachingWidth[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2] + (clamp ? 0 : FuncInline.WidthClampOffset), // 
                                     0.1);
        }
        ...
        public static bool CheckWidthInit(FuncInline.enumServoAxis axis)
        {
            return GlobalVar.AxisStatus[(int)axis].StandStill &&
                FuncMotion.CheckNearPos(GlobalVar.AxisStatus[(int)axis].Position,
                                FuncInline.TeachingWidth[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2] + FuncInline.WidthClampOffset, //??? 이거 확인좀
                                0.1);
        }

        // 오토닉스 스탭모터 초기화 되었는가? 위치값이 언클램프 위치
        public static bool CheckWidthInit(FuncInline.enumLetsAxis axis)
        {
            return CheckLetsNearPos(axis, false);
        }
        ...
        public static bool IsArrived(int axis, double posMM)
        {
            ...
            if (IsStandStill && (diff < 0.1)) return true;
            return false;
        }
        ...
        public static bool ServoInterlockCheck(int axis, double dPosMM)
        {
            ...
            if (GlobalVar.SystemStatus == enumSystemStatus.EmgStop ||
                GlobalVar.E_Stop ||
                GlobalVar.DoorOpen ||
                GlobalVar.Class == null)
            {
                FuncInline.Interlock_View = "E_Stop / Door Open Interlock";
                return false;
            }
            ...
            switch ((FuncInline.enumServoAxis)axis)
            {
                ...
                case FuncInline.enumServoAxis.SV00_In_Shuttle:
                    ...
                    //int SV00_In_Shuttle = (int)FuncInline.enumServoAxis.SV00_In_Shuttle;
                    //double current_pos = GlobalVar.AxisStatus[SV00_In_Shuttle].Position;
                    ...
                    //{
                    //    Interlock.Add("SV00_In_Shuttle 이동불가 - 컨베이어 동작중(Y01_0)");
                ...
                case FuncInline.enumServoAxis.SV02_Lift1:
                    Interlock.Clear();
                    FuncInline.Interlock_View = "";
                    int SV02_Lift1 = (int)FuncInline.enumServoAxis.SV02_Lift1;
                    bool CW = DIO.GetDOData((int)FuncInline.enumDONames.Y405_0_Front_Lift_Up_Motor_Cw);
                    bool CCW = DIO.GetDOData((int)FuncInline.enumDONames.Y405_2_Front_Lift_Up_Motor_Ccw);

                    bool interlock = DIO.GetDIData((int)FuncInline.enumDINames.X114_7_Front_Rack_PCB_Interlock_Sensor);

                    ////컨베이어 클램프 인터락 체크
                    if (CW || CCW)
                    {
                        Interlock.Add($"SV02_Lift1 cannot be moved - Conveyor is in operation {(CW ? "Y405.0 CW":"")} {(CCW ? "Y405_2 CCW" : "")}");
                        FuncInline.LogView($"SV02_Lift1 cannot be moved - Conveyor is in operation {(CW ? "Y405.0 CW" : "")} {(CCW ? "Y405_2 CCW" : "")}");
                    }
                    if (interlock)
                    {
                        Interlock.Add($"SV02_Lift1 cannot be moved - PCB interlock sensor detected. X114_7_Front_Rack_PCB_Interlock_Sensor");
                        FuncInline.LogView($"SV02_Lift1 cannot be moved - PCB interlock sensor detected. X114_7_Front_Rack_PCB_Interlock_Sensor");
                    }

                    break;
                #endregion
                #region SV03_Rack1_Width
                case FuncInline.enumServoAxis.SV03_Rack1_Width:
                    Interlock.Clear();
                    FuncInline.Interlock_View = "";
                ...
                case FuncInline.enumServoAxis.SV04_Lift2:
                    Interlock.Clear();
                    FuncInline.Interlock_View = "";
                    CW = DIO.GetDOData((int)FuncInline.enumDONames.Y305_2_Rear_Lift_Up_Motor_Cw);
                    CCW = DIO.GetDOData((int)FuncInline.enumDONames.Y304_2_Rear_Lift_Up_Motor_Ccw);

                    interlock = DIO.GetDIData((int)FuncInline.enumDINames.X114_7_Front_Rack_PCB_Interlock_Sensor);

                    ////컨베이어 클램프 인터락 체크
                    if (CW || CCW)
                    {
                        Interlock.Add($"SV04_Lift2 cannot be moved - Conveyor is in operation {(CW ? "Y305.2 CW" : "")} {(CCW ? "Y304.2 CCW" : "")}");
                        FuncInline.LogView($"SV04_Lift2 cannot be moved - Conveyor is in operation {(CW ? "Y305.2 CW" : "")} {(CCW ? "Y304.2 CCW" : "")}");
                    }
                    if (interlock)
                    {
                        Interlock.Add($"SV04_Lift2 cannot be moved - PCB interlock sensor detected. X114_7_Front_Rack_PCB_Interlock_Sensor");
                        FuncInline.LogView($"SV04_Lift2 cannot be moved - PCB interlock sensor detected. X114_7_Front_Rack_PCB_Interlock_Sensor");
                    }
                    break;
                #endregion
                #region SV05_Rack2_Width
                case FuncInline.enumServoAxis.SV05_Rack2_Width:
                    Interlock.Clear();
                    FuncInline.Interlock_View = "";
