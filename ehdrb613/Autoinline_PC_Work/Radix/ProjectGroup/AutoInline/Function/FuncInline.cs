   ...
   */
    ...
    public class FuncInline
    {
        ...
        public enum enumDINames
        {
            ...
            X302_5_Out_Shuttle_Stopper_Cyl_Out_Sensor, //OK,NG라인 동시 동작
        ...
        public enum enumDONames
        {
            ...
            Y300_2_Out_Shuttle_CONTACT_STOPPER_Out_SOL,
            Y302_1_Out_Shuttle_CONTACT_STOPPER_In_SOL,
        ...
        public enum enumSMDStatus // SMD 테스트 상태. PCB 상태만 지정
        {
            ...
            InConveyor, //인컨베이어 투입중
            Before_Scan, // 스캔 전이다
            Cooling, // 쿨링중이다
            Bypass, //Pass모드이거나 직렬 Pass일때
            ...
            //==========================
            //D1,D2지그 DL,FT(Test_Pass/Test_Fail) 분리형 전용
            DTest_Pass, // 양품 판정
            DTest_Fail, // 비품 판정
            DTest_AllFail,
            //==========================
        ...
        public enum enumLiftPos // 리프트 위치. Lift2 경우도 site1부터 할당
        {
            ...
            RearInShuttlePos    //RearLift-InShuttle로딩위치
        ...
        public enum enumLetsMotion // Lets보드 모션 컨트롤러 순번
        {
        ...
        public enum enumLetsAxis // Autonics PMC Motion 컨트롤러에 연결된 축 이름, 짝수면 x, 홀수면 y
        {
        ...
        {
            public enumLetsMotion Motion;
            public enumAxis Axis;
            public enumletsCommand Command;
            public double Position;
            public int Velocity;
            public int Acc;
            public int Dec;
        ...
        public static class SiteIoMaps
        {
            ...
            public static bool TryGetSiteMotor(enumTeachingPos site, out enumDONames cw, out enumDONames ccw)
            {
                switch (site)
                {
                    // Front DT
            ...
            public static bool TryGetPcbDockDI(enumTeachingPos site, out enumDINames di)
            {
                ...
                //5세대 이상일경우 센서 확인 필요 Function4층과 DL10층이 바뀜
            }
        ...
        public static void Start_Button(bool op)
        {
            ...
            if (GlobalVar.SystemStatus == enumSystemStatus.Manual)
            {
                ...
                ((AutoInline)GlobalVar.Class).runTime = GlobalVar.TickCount64;
        ...
        public static void Stop_Button()
        {
            ...
            if (GlobalVar.SystemStatus == enumSystemStatus.Manual || GlobalVar.SystemStatus >= enumSystemStatus.AutoRun)
            {
                GlobalVar.SystemStatus = enumSystemStatus.Manual;
            }
            else
            ...
            ((AutoInline)GlobalVar.Class).runTotal += (GlobalVar.TickCount64 - ((AutoInline)GlobalVar.Class).runTime) / 1000;
        ...
        public static void InitAllSubAction()
        {
            ...
            AutoInline.Class.InShuttle.Action = InShuttle.enumAction.Waiting;    //inshuttle 상태 변경
            AutoInline.Class.FrontRack.Action = FrontRack.enumAction.Waiting;    //FrontRack 상태 변경
            AutoInline.Class.FrontRack.PasslineAction = FrontRack.enumPassLineAction.Waiting;    //FrontRack 상태 변경
            AutoInline.Class.FrontRack.ScanSiteAction = FrontRack.enumScanSiteAction.Waiting;    //FrontRack 상태 변경
            AutoInline.Class.RearRack.Action = RearRack.enumAction.Waiting;    //RearRack 상태 변경
            AutoInline.Class.OutShuttle.OutShuttleAction = OutShuttle.OutShuttle_enumAction.Waiting;    //OutShuttle 상태 변경
            AutoInline.Class.OutShuttle.OutConveyorAction = OutShuttle.OutConveyor_enumAction.Waiting;    //OutConveyor 상태 변경
            AutoInline.Class.OutShuttle.NgbufferAction = OutShuttle.Ngbuffer_enumAction.Waiting;    //Ngbuffer 상태 변경
            AutoInline.Class.Scan.Action = Scan.enumAction.Waiting;    //Scan 상태 변경
        }
        #endregion
       ...
       // public static double[] OffsetWidth = new double[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2]; // 스텝모터 5, 서보2 생성시 변수로 지정하면 0이 되어 상수로 입력
        ...
        public static int MaxTestPCCount = 9;        // 각 Test PC의 갯수
        //{
        //    get
        //    {
        //        switch (InlineType)
        ...
        public static bool[] WidthStarted = new bool[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2]; // 폭 Initialize 진행 단계, 오토닉스 + 사이트 + 지그버퍼폭. 생성시 변수로 지정하면 0이 되어 상수로 입력
        public static bool[] WidthDone = new bool[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2]; // Initialize 진행 단계 완료 여부, 오토닉스 + 사이트 + 지그버퍼폭. 생성시 변수로 지정하면 0이 되어 상수로 입력
        ...
        public static double Temperature_NUX_PV = 0; // 온도 값(한영넉스) 모듈인라인
        public static double Temperature_NUX_SV = 0; // 온도 값(한영넉스) 모듈인라인
        public static double Temperature_NS10LT_PV = 0; // 온도 값(NS10LT) 모듈인라인
        ...
        public static int PortScanner = 10;
        ...
        public static int[] PortPMC = new int[Enum.GetValues(typeof(FuncInline.enumLetsMotion)).Length]; // 컨베어 폭조절 모터 통신 포트
        public static int[] NodePMC = new int[Enum.GetValues(typeof(FuncInline.enumLetsMotion)).Length]; // 컨베어 폭조절 모터 Node ID
        public static int[] BaudTest = new int[ComSMDCount]; // Test PC  통신 속도
        public static int[] BaudPMC = new int[Enum.GetValues(typeof(FuncInline.enumLetsMotion)).Length]; // 컨베어 폭조절 모터  통신 속도
        public static Parity[] ParityTest = new Parity[ComSMDCount]; // Test PC  통신 Parity
        public static Parity[] ParityPMC = new Parity[Enum.GetValues(typeof(FuncInline.enumLetsMotion)).Length]; // 컨베어 폭조절 모터 통신 Parity
        public static StopBits[] StopBitsTest = new StopBits[ComSMDCount]; // Test PC  통신 Stopbit
        public static StopBits[] StopBitsPMC = new StopBits[Enum.GetValues(typeof(FuncInline.enumLetsMotion)).Length]; // 컨베어 폭조절 모터 통신 Stopbit
        ...
        public static double[] TeachingWidth = new double[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2]; // 폭조절 티칭값, 스텝모터 + 사이트 + 서보. 생성시 변수로 지정하면 0이 되어 상수로 입력
        public static double[] OffsetWidth = new double[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2]; // 폭조절 옵셋값. 실제위치와 폭을 일치, 스텝모터 + 사이트 + 서보. 생성시 변수로 지정하면 0이 되어 상수로 입력
        public static bool[] WidthDisplayReverse = new bool[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2]; // [axis]별 표시 역방향 여부 (기본 false)
        ...
        public static enumTeachingPos[] SiteFTOrder = // 펑션투입 순서. 좌우 번갈아
       {
        ...
        public static Stopwatch[] SiteCheckTime = new Stopwatch[26]; // 각 사이트 PCB 공급 후 경과 시간. 동작시작 전에 start, 동작 완료 후 reset
        ...
        public static bool[] InputStop = new bool[2]; // Rack별 투입 정지
        public static bool OutputStop = false; // 뒷 라인으로 배출 금지
        public static bool InStop = false; // 앞 라인에서 투입 금지
        ...
        // 모든 타이머를 관리하는 딕셔너리
        private static Dictionary<string, Stopwatch> delayTimers = new Dictionary<string, Stopwatch>();

        /// <summary>
        /// 특정 이름(Key)을 가진 타이머가 설정된 시간(delayMs)을 넘겼는지 확인합니다.
        /// </summary>
        /// <param name="key">타이머 이름 (예: "Front_Passline", "Rear_Lift")</param>
        /// <param name="delayMs">대기 시간 (ms)</param>
        /// <returns>True: 시간 경과됨 / False: 아직 안됨</returns>
        public static bool IsDelayOver(string key, long delayMs)
        {
            // 1. 해당 키의 타이머가 없으면 생성 및 시작
            if (!delayTimers.ContainsKey(key))
            {
                delayTimers[key] = new Stopwatch();
                delayTimers[key].Start();
                return false; // 방금 시작했으니 아직 시간 안됨
            }

            // 2. 시간 체크
            if (delayTimers[key].ElapsedMilliseconds > delayMs)
            {
                delayTimers[key].Stop();
                delayTimers.Remove(key); // 사용 완료 후 삭제 (자동 리셋 효과)
                return true; // 완료
            }
        ...
        /// <summary>
        /// 특정 이름의 타이머를 강제 초기화합니다. (에러, 정지 시 사용)
        /// </summary>
        public static void ResetDelay(string key)
        {
            if (delayTimers.ContainsKey(key))
            {
                delayTimers.Remove(key);
            }
        }

        /// <summary>
        /// 모든 타이머 초기화 (초기화, 에러 시 사용)
        /// </summary>
        public static void ResetAllDelays()
        {
            delayTimers.Clear();
        }
        ...
        #region 스텝모터 변수
        public static FuncInline.structPMCStatus[] PMCStatus = new FuncInline.structPMCStatus[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length]; // Lets 스텝모터 축 상태 저장
        ...
        public static bool LongPCBTest = false; // 250mm의 긴 PCB 사용여부(IN 센서 무시하기위해)
...
SELECT [Date]
  ...
  //*/
        ...
        #region PCB 운영 관련
        /// <summary>
        /// 장비 내 PCB 존재 여부를 확인하고, 존재 시 메시지 박스를 띄웁니다.
        /// (Data 상태와 물리 센서 모두 확인)
        /// </summary>
        /// <returns>true: PCB 있음, false: PCB 없음</returns>
        public static bool CheckPCBInMachine()
        {
            List<string> detectedList = new List<string>(); // 감지된 내역 저장용 리스트

            try
            {
                // =================================================================
                // 1. [Logic Check] PCB Data 정보 확인 (소프트웨어 메모리 상의 잔류)
                // =================================================================

                // (1) Teaching Sites (Rack) 확인
                for (int i = 0; i < MaxSiteCount; i++)
                {
                    int siteIndex = (int)enumTeachingPos.Site1_F_DT1 + i;
                    if (FuncInline.PCBInfo[siteIndex].PCBStatus != FuncInline.enumSMDStatus.UnKnown)
                    {
                        string siteName = ((enumTeachingPos)siteIndex).ToString();
                        detectedList.Add($"[Data] {siteName}");
                    }
                }

                // (2) 기타 버퍼 구간 확인
                // 확인하고 싶은 위치들을 배열로 정의하여 반복문 처리 (코드 단축)
                enumTeachingPos[] bufferPositions = new enumTeachingPos[]
                {
                    enumTeachingPos.InConveyor,
                    enumTeachingPos.InShuttle,
                    enumTeachingPos.NgBuffer,
                    enumTeachingPos.OutShuttle_Up,
                    enumTeachingPos.OutConveyor,
                    enumTeachingPos.FrontPassLine,
                    enumTeachingPos.RearPassLine,
                    enumTeachingPos.RearNGLine,
                    enumTeachingPos.Lift1_Up,
                    enumTeachingPos.Lift1_Down,
                    enumTeachingPos.Lift2_Up,
                    enumTeachingPos.Lift2_Down,
                    enumTeachingPos.FrontScanSite
                };

                foreach (var pos in bufferPositions)
                {
                    if (PCBInfo[(int)pos].PCBStatus != FuncInline.enumSMDStatus.UnKnown)
                    {
                        detectedList.Add($"[Data] {pos}");
                    }
                }

                // =================================================================
                // 2. [Physical Check] 실제 센서 감지 확인 (DI 이름 문자열 검색)
                // =================================================================
                for (int i = 0; i < Enum.GetValues(typeof(FuncInline.enumDINames)).Length; i++)
                {
                    string di_name = ((FuncInline.enumDINames)i).ToString();

                    // 센서 이름에 특정 키워드가 포함되어 있는지 확인
                    ...
                    {
                        // 센서가 켜져있다면(True) 리스트에 추가
                        if (DIO.GetDIData(i))
                        {
                            detectedList.Add($"[Sensor] {di_name}");
                        }
                    }
                }
                ...
                if (detectedList.Count > 0)
                {
                    ...
                    string finalMsg = $"PCB Detected in Machine!\n\n{msgBody}\n\nPlease check and remove.";
                ...
                {
                    return false; // PCB 없음 (깨끗함)
                }
            }
            catch (Exception ex)
            {
                FuncLog.WriteLog("CheckPCBInMachine Error : " + ex.Message);
                // 에러 발생 시 안전을 위해 true 반환(동작 막음)하거나 메시지 띄움
                FuncWin.TopMessageBox("CheckPCBInMachine Exception Occurred.");
                return true;
            }
        }
        ...
        public static bool CheckSiteFull(bool reTest)
        {
            ...
            try
            {
                ...
                if (!reTest)
                {
                    for (int i = (int)enumTeachingPos.Site1_F_DT1; i <= (int)enumTeachingPos.Site13_F_FT3; i++)
                    {
        ...
        public static bool CheckPCBSensor()
        {
            // 감지된 구역 목록
        ...
        public static void WriteSystemLog_Samsung() // jhryu 20230220 삼성요구
        {
            ...
            try
            {
                ...
                int maxModuleCount = UseSite.Length;   //26
                int maxArrayCount = MaxArrayCount;     //6
                ...
                FuncFile.WriteIniFile(section, "Width HomSpeed", GlobalVar.WidthHomSpeed.ToString("F0"), logPath);
                FuncFile.WriteIniFile(section, "Width Speed", GlobalVar.WidthSpeed.ToString("F0"), logPath);
                FuncFile.WriteIniFile(section, "Width AccDec", GlobalVar.WidthAccDec.ToString("F0"), logPath);
        ...
        public static bool InShuttle_Input()
        {
            try
            {
                ...
                DIO.WriteDOData(enumDONames.Y4_2_SMEMA_Before_Ready, !FuncInline.CycleStop &&
                                                                        ...
                                                                        FuncInlineMove.CheckNearPMCWidth(FuncInline.enumLetsAxis.ST00_InShuttle_Width) &&
                                      ...
                                      System.Diagnostics.Stopwatch actionWatch, bool passMode)
        {
            try
            {
                ...
                // 맵핑
                if (!SiteIoMaps.TryGetSiteMotor(destSite, out var siteCw, out var siteCcw)) return false;
        ...
        public static bool CheckLiftPos(enumLiftName lift, FuncInline.enumLiftPos pos, double gap)
        {
            try
            {
                ...
                double axis_pos = GlobalVar.AxisStatus[axis].Position; //FuncMotion.GetRealPosition(axis);
        ...
        public static bool MoveLift(enumLiftName lift, FuncInline.enumLiftPos pos, double speed) // 투입리프트 동작. 지정속도로
        {
            try
            {
                ...
                if (lift == enumLiftName.FrontLift)
                {
                    ...
                    {
                        enumDONames cw, ccw;
                        if (SiteIoMaps.TryGetSiteMotor(site, out cw, out ccw)
        ...
        // all false : 사이트만
        // all true : 장비 전체
        public static int GetPCBCount(bool all)
        {
            int sitePCB = 0;
            int totalPCB = 0;
            for (int siteIndex = 0; siteIndex < FuncInline.MaxSiteCount; siteIndex++)
            {
                //Front
                if (siteIndex <= 12)
                {
                    if (FrontRack.Front_PCB_Sensor[siteIndex] &&
                    PCBInfo[(int)enumTeachingPos.Site1_F_DT1 + siteIndex].PCBStatus != enumSMDStatus.UnKnown)
                    {
                        sitePCB++;
                        totalPCB++;
                    }
                ...
                {
                    if (RearRack.Rear_PCB_Sensor[siteIndex -13] &&
                   PCBInfo[(int)enumTeachingPos.Site14_R_DT1 + (siteIndex - 13)].PCBStatus != enumSMDStatus.UnKnown)
                    {
                        sitePCB++;
                        totalPCB++;
                    }
            ...
            if (DIO.GetDIData(FuncInline.enumDINames.X114_6_Front_PASSLINE_PCB_Stop_Sensor) &&
                 PCBInfo[(int)enumTeachingPos.FrontPassLine].PCBStatus != enumSMDStatus.UnKnown)
            {
                totalPCB++;
            }
            ...
            if ((DIO.GetDIData(FuncInline.enumDINames.X404_6_Rear_Lift_Up_PCB_In_Sensor) ||
                 ...
                 PCBInfo[(int)enumTeachingPos.Lift2_Up].PCBStatus != enumSMDStatus.UnKnown)
            {
                totalPCB++;
            }
            if (DIO.GetDIData(FuncInline.enumDINames.X03_4_Front_Scan_PCB_Dock_Sensor) &&
                 PCBInfo[(int)enumTeachingPos.FrontScanSite].PCBStatus != enumSMDStatus.UnKnown)
            {
                totalPCB++;
            }
         
            // OutShuttle은 정보에 따라 분기해야 함
            ...
            if ((DIO.GetDIData(FuncInline.enumDINames.X02_3_Out_Conveyor_PASSLIne_PCB_Start_Sensor) ||
              ...
              PCBInfo[(int)enumTeachingPos.OutConveyor].PCBStatus != enumSMDStatus.UnKnown)
            {
                totalPCB++;
            }
            if ((DIO.GetDIData(FuncInline.enumDINames.X402_2_Out_Conveyor_NG_PCB_In_Sensor) ||
                    DIO.GetDIData(FuncInline.enumDINames.X04_3_Out_Conveyor_NG_PCB_Stop_Sensor)) &&
        ...
        // 유호 사이트 갯수 구하기
        public static int GetValidSiteCount()
        {
            int cnt = 0;
            for (int i = 0; i < MaxSiteCount; i++)
            {
                if (SiteState[i] == enumSiteState.Valid &&
                    InitialDone[(int)enumInitialize.Site1_F_DT1 + i] == true)
                {
                    cnt++;
                }
            }
        ...
        /// <summary>
        /// 해당 랙(Front/Rear)에 PCB 투입이 가능한지 판단합니다.
        /// (Lift 상태 + 세대별 유효 사이트 중 빈 곳이 있는지 체크)
        /// </summary>
        /// <param name="isFront">true: Front Rack, false: Rear Rack</param>
        /// <returns>true: 투입 가능, false: 투입 불가(Full)</returns>
        public static bool IsRackReady(bool isFront)
        {
            // 0. Destination 초기화
            FuncInline.PCBInfo[(int)FuncInline.enumTeachingPos.InShuttle].Destination = FuncInline.enumTeachingPos.None;

            // [추가] 1. InputStop(투입 정지) 확인
            // 0: Front, 1: Rear -> true면 투입 금지
            int rackIdx = isFront ? 0 : 1;
            if (FuncInline.InputStop[rackIdx])
            {
                return false; // 해당 Rack 투입 정지 상태
            }


            // 1. [Physical Check] 물리적 진입 경로(Lift, PassLine) 확인
            var liftPos = isFront ? FuncInline.enumTeachingPos.Lift1_Up : FuncInline.enumTeachingPos.Lift2_Up;

            if (FuncInline.PCBInfo[(int)liftPos].PCBStatus != FuncInline.enumSMDStatus.UnKnown)
            {
                return false; // 리프트에 이미 제품이 있음
            }

            if (isFront && FuncInline.PCBInfo[(int)FuncInline.enumTeachingPos.FrontPassLine].PCBStatus != FuncInline.enumSMDStatus.UnKnown)
            {
                return false; // FrontPassline 막힘
            }

            // 2. [Range Setting] 세대별 사용할 사이트 개수 설정
            int validSiteCount = 0;

            if (FuncInline.InlineType <= FuncInline.enumInlineType.Gen4)
            {
                validSiteCount = 10; // Gen1 ~ Gen4
            }
            else if (FuncInline.InlineType == FuncInline.enumInlineType.Gen5)
            {
                validSiteCount = 9;  // Gen5
            }
            else
            {
                validSiteCount = 13; // Gen6 이상
            }

            // 3. [Circular Search] 순차 검색 로직
            // 마지막 위치(LastIndex) 다음 칸부터 검색을 시작합니다.

            // 현재 Rack의 지난번 위치 가져오기
            int lastIndex = isFront ? AutoInline.Class.InShuttle.LastFrontIndex : AutoInline.Class.InShuttle.LastRearIndex;

            // Front/Rear 시작 Teaching Position
            int startBaseIdx = isFront ? (int)FuncInline.enumTeachingPos.Site1_F_DT1 : (int)FuncInline.enumTeachingPos.Site14_R_DT1;

            // 유효 개수만큼 반복 (한 바퀴 돌기)
            for (int k = 1; k <= validSiteCount; k++)
            {
                // (지난번위치 + k) % 전체개수 => 순환 인덱스 계산
                // 예: valid=10, last=9(10층) -> next는 (9+1)%10 = 0(1층)
                int offset = (lastIndex + k) % validSiteCount; 

                int currentSiteIdx = startBaseIdx + offset;

                // 1. UseSite[idx]가 true (사용함) 이어야 함
                // 2. PCBStatus가 UnKnown (비어있음) 이어야 함
                // 해당 사이트가 비어있다면(UnKnown)
                if (FuncInline.UseSite[currentSiteIdx] &&
                    FuncInline.PCBInfo[currentSiteIdx].PCBStatus == FuncInline.enumSMDStatus.UnKnown)
                {
                    // [할당] 목적지 설정
                    FuncInline.PCBInfo[(int)FuncInline.enumTeachingPos.InShuttle].Destination = (FuncInline.enumTeachingPos)currentSiteIdx;

                    // [기록] 이번에 찾은 위치를 LastIndex로 업데이트 (다음번엔 여기 다음부터 찾음)
                    if (isFront)
                        AutoInline.Class.InShuttle.LastFrontIndex = offset;
                    else
                        AutoInline.Class.InShuttle.LastRearIndex = offset;
        ...
        /// <summary>
        /// 해당 랙(Front/Rear)에 투입 가능한 빈 사이트를 찾아서 반환합니다.
        /// </summary>
        ...
        public static FuncInline.enumTeachingPos GetAvailableSite(bool isFront)
        {
            ...
            int rackIdx = isFront ? 0 : 1;
            if (FuncInline.InputStop[rackIdx]) return FuncInline.enumTeachingPos.None;

            // 2. [Physical Check] 물리적 진입 경로 확인
            var liftPos = isFront ? FuncInline.enumTeachingPos.Lift1_Up : FuncInline.enumTeachingPos.Lift2_Up;

            // 리프트에 제품이 있거나 Waiting 중이 아닐때
            if (FuncInline.PCBInfo[(int)liftPos].PCBStatus != FuncInline.enumSMDStatus.UnKnown && 
                (isFront ? AutoInline.Class.FrontRack.Action == FrontRack.enumAction.Waiting
                : AutoInline.Class.RearRack.Action == RearRack.enumAction.Waiting))
                return FuncInline.enumTeachingPos.None;
           
                //(isFront ? FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV02_Lift1, LiftPos[rackIdx, (int)enumLiftPos.FrontPassLine])
                //: FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV02_Lift1, LiftPos[rackIdx, (int)enumLiftPos.FrontPassLine])
            ...
            // 3. [Range Setting]
            int validSiteCount = 0;
            if (FuncInline.InlineType <= FuncInline.enumInlineType.Gen4) validSiteCount = 10;
            else if (FuncInline.InlineType == FuncInline.enumInlineType.Gen5) validSiteCount = 9;
            else validSiteCount = 13;
            ...
            for (int k = 1; k <= validSiteCount; k++)
            {
                ...
                if (FuncInline.UseSite[useSiteIdx] &&
                    FuncInline.PCBInfo[currentSiteIdx].PCBStatus == FuncInline.enumSMDStatus.UnKnown)
                {
                    // 여기서 LastIndex를 업데이트 하지 않습니다. (확정된 게 아니므로)
                    // 호출하는 쪽에서 확정하면 그때 업데이트 하거나, 
                    // 단순 조회용이라면 여기서 업데이트 하면 안됩니다. 
                    // 하지만 로직상 여기서 찾았으면 바로 쓸 것이므로 업데이트 해도 무방하나,
                    // 안전하게 확정 시 업데이트 하려면 offset을 별도로 리턴해야 합니다.
                    // 편의상 여기서는 위치만 리턴합니다.
