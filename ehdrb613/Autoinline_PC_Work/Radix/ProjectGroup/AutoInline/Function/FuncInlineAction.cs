...
namespace Radix
{
    public class FuncInlineAction
    {
        ...
        /// <summary>
        /// 리프트/사이트의 동작을 단계별(Loading, Unclamp, Unloading)로 실행합니다.
        /// (Sleep 대신 Stopwatch 사용, 컨베이어 구동 중 클램프 로직 포함)
        /// </summary>
        public static bool OneSiteMove(enumTeachingPos site, enumSiteAction action, int timeOutMs = 10000)
        {
            // 1. I/O 매핑 정보 가져오기
            enumDINames diPcbSensor;      // PCB 감지 센서
            ...
            if (!FuncInline.SiteIoMaps.TryGetPcbDockDI(site, out diPcbSensor) ||
                ...
                !FuncInline.SiteIoMaps.TryGetContactUpDownDO(site, out doContactUpDown) ||
                !FuncInline.SiteIoMaps.TryGetContactUpDI(site, out diContactUpSensor))
            {
                FuncLog.WriteLog($"[{site}] I/O Mapping Error.");
                return false;
            }
            ...
            try
            {
                switch (action)
                {
                    // =================================================================================
                    // Case 1: Loading (진입)
                    // 시퀀스: CCW 진입 -> 센서감지 -> (컨베이어 유지) -> 클램프 ON -> 컨베이어 정지 -> 포고핀 다운 -> 다운확인
                    // =================================================================================
                    case enumSiteAction.Loading:
                        ...
                        if (!WaitForSensor(diPcbSensor, true, timeOutMs))
                        {
                            ...
                            FuncLog.WriteLog($"[{site}] [Loading] Fail: Timeout (Sensor Not Detected)");
                            return false;
                        }
                        FuncLog.WriteLog($"[{site}] [Loading] Sensor Detected");

                        // 1-3. [중요] 컨베이어 가동 중 클램프 동작 (PCB 틀어짐 방지)
                        DIO.WriteDOData((int)doClampStopper, true);
                        ...
                        // 1-4. 컨베이어 정지
                        ControlConveyor(site, enumMotorDir.Stop);
                        if (!WaitMs(100)) return false;

                        // 1-5. 포고핀 다운
                        DIO.WriteDOData((int)doContactUpDown, true);
                        FuncLog.WriteLog($"[{site}] [Loading] Contact Down Command");

                        // 1-6. 포고핀 다운 확인 (Up 센서가 꺼져야 함)
                        if (!WaitMs(500)) return false; // 이동 시간

                        // Up 센서가 OFF 될 때까지 대기 (다운 완료 체크)
                        if (!WaitForSensor(diContactUpSensor, false, 3000))
                        {
                            FuncLog.WriteLog($"[{site}] [Loading] Fail: Contact Down Check Fail (Up Sensor Still ON)");
                            return false;
                        }
                        // 1-7. 클램프 해제
                        DIO.WriteDOData((int)doClampStopper, false);

                        FuncLog.WriteLog($"[{site}] [Loading] Test Ready");
                        return true;


                    // =================================================================================
                    // Case 2: Unclamp (테스트 완료 후 해제)
                    // 시퀀스: 포고핀 업 -> 업 확인 -> 클램프 해제 -> (대기)
                    // =================================================================================
                    case enumSiteAction.Tested:
                        FuncLog.WriteLog($"[{site}] [Unclamp] Start");

                        // 2-1. 포고핀 업
                        DIO.WriteDOData((int)doContactUpDown, false);
                        FuncLog.WriteLog($"[{site}] [Unclamp] Contact Up Command");

                        // 2-2. 포고핀 업 확인 (Up 센서가 켜져야 함)
                        if (!WaitMs(500)) return false; // 이동 시간

                        if (!WaitForSensor(diContactUpSensor, true, 3000))
                        {
                            FuncLog.WriteLog($"[{site}] [Unclamp] Fail: Contact Up Check Fail (Up Sensor Not Detected)");
                            return false;
                        }
                        FuncLog.WriteLog($"[{site}] [Unclamp] Contact Up Checked");

                        // 2-3. 클램프 해제
                        DIO.WriteDOData((int)doClampStopper, false);
                        FuncLog.WriteLog($"[{site}] [Unclamp] Clamp OFF");

                        if (!WaitMs(200)) return false;

                        FuncLog.WriteLog($"[{site}] [Unclamp] Complete (Waiting for Unloading)");
                        return true;


                    // =================================================================================
                    // Case 3: Unloading (배출)
                    // 시퀀스: CW(역방향) 배출 -> 센서꺼짐 확인 -> 정지
                    // =================================================================================
                    case enumSiteAction.Unloading:
                        ...
                        if (!WaitForSensor(diPcbSensor, false, timeOutMs))
                        {
                            ControlConveyor(site, enumMotorDir.Stop);
                            FuncLog.WriteLog($"[{site}] [Unloading] Fail: Timeout (Sensor Still ON)");
                            return false;
                        }
                        ...
                        // 3-4. 컨베이어 정지
                        ControlConveyor(site, enumMotorDir.Stop);
                        FuncLog.WriteLog($"[{site}] [Unloading] Complete");
                        return true;
            ...
            {
                ControlConveyor(site, enumMotorDir.Stop);
                FuncLog.WriteLog($"[{site}] Action Error: {ex.Message}");
                return false;
            }
        }
        /// <summary>
        /// 특정 사이트의 컨베이어 모터를 제어합니다. (인터락 적용: 반대 방향 정지 후 동작)
        /// </summary>
        /// <param name="site">대상 사이트 위치</param>
        /// <param name="direction">동작 명령 (Stop, CW, CCW)</param>
        public static void ControlConveyor(enumTeachingPos site, enumMotorDir direction)
        {
            enumDONames doCW, doCCW;

            // 1. 해당 사이트의 모터 DO 매핑 가져오기
            // (TryGetSiteMotor가 다른 클래스에 있다면 '클래스명.TryGetSiteMotor'로 호출하세요)
            if (SiteIoMaps.TryGetSiteMotor(site, out doCW, out doCCW))
            {
                switch (direction)
                {
                    case enumMotorDir.Stop:

                        DIO.WriteDOData((int)doCW, false);
                        DIO.WriteDOData((int)doCCW, false);
                        break;

                    case enumMotorDir.CW:

                        DIO.WriteDOData((int)doCCW, false);
                        DIO.WriteDOData((int)doCW, true);
                        break;

                    case enumMotorDir.CCW:

                        DIO.WriteDOData((int)doCW, false);
                        DIO.WriteDOData((int)doCCW, true);
                        break;
                }
            }
            else
            {
                // 매핑되지 않은 사이트가 들어온 경우 로그 출력 등 예외 처리
                // FuncLog.WriteLog($"Error: {site} is not defined in TryGetSiteMotor.");
            }
        }
        // ---------------------------------------------------------
        // [Helper] Sleep 대신 사용하는 Non-blocking Delay 함수
        // ---------------------------------------------------------
        private static bool WaitMs(int ms)
        {
            ...
            {
                if (GlobalVar.GlobalStop) return false; // 정지 신호 시 즉시 중단
                Thread.Sleep(1); // CPU 점유율 방지용 최소 Sleep
            }
            return true;
        }

        // ---------------------------------------------------------
        // [Helper] 센서 상태 대기 함수 (Stopwatch 사용)
        // targetState: true(ON 대기), false(OFF 대기)
        // ---------------------------------------------------------
        private static bool WaitForSensor(enumDINames sensor, bool targetState, int timeoutMs)
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();
            while (sw.ElapsedMilliseconds < timeoutMs)
            {
                if (GlobalVar.GlobalStop) return false;

                if (DIO.GetDIData(sensor) == targetState)
                {
                    return true;
                }
                Thread.Sleep(5);
        ...
        public static bool SitePCBLoad(FuncInline.enumTeachingPos site)
        {
            ...
            try
            {
                ...
                bool hasMotors = FuncInline.SiteIoMaps.TryGetSiteMotor(site, out motorCw, out motorCcw);
        ...
        public static bool CheckOriginDone(FuncInline.enumInitialize part)
        {
                ...
                part <= FuncInline.enumInitialize.Site26_R_FT3)
            {
                ...
                if (FuncInline.SiteIoMaps.TryGetSiteMotor(sitePos, out var cwDo, out var ccwDo))
                {
            ...
            switch (part)
            {
                case FuncInline.enumInitialize.InConveyor:
                    return GlobalVar.LetsAxisStatus[(int)FuncInline.enumLetsAxis.ST03_InConveyor_Width].isHomed;

                case FuncInline.enumInitialize.InShuttle:
                    return (GlobalVar.AxisStatus[(int)FuncInline.enumServoAxis.SV00_In_Shuttle].isHomed && 
                            FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV00_In_Shuttle, 0) &&
                            //GlobalVar.AxisStatus[(int)FuncInline.enumServoAxis.SV00_In_Shuttle].HomeAbsSwitch &&
                            GlobalVar.LetsAxisStatus[(int)FuncInline.enumLetsAxis.ST00_InShuttle_Width].isHomed &&
                ...
                case FuncInline.enumInitialize.OutShuttle:
                    return (GlobalVar.AxisStatus[(int)FuncInline.enumServoAxis.SV01_Out_Shuttle].isHomed &&
                        FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV01_Out_Shuttle, 0) &&
                    GlobalVar.LetsAxisStatus[(int)FuncInline.enumLetsAxis.ST01_OutShuttle_Width].isHomed); 

                                                                    //!DIO.GetDIData(FuncInline.enumDINames.X303_5_Out_Shuttle_Stopper_Cyl_IN_Sensor) &&
                                                                    //!DIO.GetDIData(FuncInline.enumDINames.X302_5_Out_Shuttle_Stopper_Cyl_Out_Sensor) &&
                ...
                case FuncInline.enumInitialize.OutConveyor:
                    return GlobalVar.LetsAxisStatus[(int)FuncInline.enumLetsAxis.ST02_OutConveyor_Width].isHomed &&
                                                                        !DIO.GetDORead((int)FuncInline.enumDONames.Y400_1_Out_Conveyor_Motor_Cw);
                case FuncInline.enumInitialize.NgBuffer:
                    return (!DIO.GetDORead((int)FuncInline.enumDONames.Y402_5_Out_Conveyor_Ng_Motor_Cw) &&
                                                                        ...
                                                                        : true) &&
                                                                         (FuncInline.InlineType >= FuncInline.enumInlineType.Gen5
                                                                        ? GlobalVar.LetsAxisStatus[(int)FuncInline.enumLetsAxis.ST04_NGBuffer].isHomed
                                                                        : true));
                case FuncInline.enumInitialize.FrontLift:
                    return (GlobalVar.AxisStatus[(int)FuncInline.enumServoAxis.SV02_Lift1].isHomed &&
                        FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV02_Lift1, 0) &&
                        GlobalVar.AxisStatus[(int)FuncInline.enumServoAxis.SV03_Rack1_Width].isHomed &&
                        FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV03_Rack1_Width, FuncInline.DefaultPCBWidth) &&
                ...
                case FuncInline.enumInitialize.RearLift:
                    return (GlobalVar.AxisStatus[(int)FuncInline.enumServoAxis.SV04_Lift2].isHomed &&
                        FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV04_Lift2, 0) &&
                        GlobalVar.AxisStatus[(int)FuncInline.enumServoAxis.SV05_Rack2_Width].isHomed &&
                        FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV05_Rack2_Width, FuncInline.DefaultPCBWidth) &&
                ...
                case FuncInline.enumInitialize.Scan:
                    return GlobalVar.AxisStatus[(int)FuncInline.enumServoAxis.SV06_Scan_Y].isHomed &&
                        FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV06_Scan_Y, 0) &&
                                                                    GlobalVar.AxisStatus[(int)FuncInline.enumServoAxis.SV07_Scan_X].isHomed &&
                                                                    FuncInlineMove.IsArrived((int)FuncInline.enumServoAxis.SV07_Scan_X, 0);
        ...
        public static void RunOrigin(FuncInline.enumInitialize part)
        {
            ...
            FuncInline.enumLetsAxis stepAxis = FuncInline.enumLetsAxis.ST00_InShuttle_Width;
            int motion_index = 0;
            ...
            switch (part)
            {
                ...
                case FuncInline.enumInitialize.InShuttle:
                        ...
                        DIO.GetDIData(FuncInline.enumDINames.X302_7_In_Shuttle_Turn_Ccw_Cyl_Sensor))
                    {
                        ...
                        #endregion
                        #region 폭조절
                        stepAxis = FuncInline.enumLetsAxis.ST00_InShuttle_Width;
                        motion_index = (int)((int)stepAxis / 2);
                        // 홈도 언클램프도 아니면 호밍
                        if (!GlobalVar.LetsAxisStatus[(int)stepAxis].isHomed &&
                            //!GlobalVar.LetsAxisStatus[(int)stepAxis].Home && // 홈 센서 감지 안 되면
                            GlobalVar.LetsAxisStatus[(int)stepAxis].StandStill)
                        {
                ...
                case FuncInline.enumInitialize.NgBuffer:
                    ...
                    #region 폭조절
                    stepAxis = FuncInline.enumLetsAxis.ST04_NGBuffer;
                    motion_index = (int)((int)stepAxis / 2);
                    // 홈도 언클램프도 아니면 호밍
                    if (//!FuncInline.CheckNearPos(stepAxis, false) && // 언클램프 위치
                        !GlobalVar.LetsAxisStatus[(int)stepAxis].isHomed && // 홈 위치 아니거나
                        GlobalVar.LetsAxisStatus[(int)stepAxis].StandStill)
                    {
                        ////FuncInline.ComPMC[motion_index].HomeRun(stepAxis, GlobalVar.WidthHomSpeed);
                    }
                ...
                case FuncInline.enumInitialize.OutShuttle:
                        ...
                        DIO.GetDIData(FuncInline.enumDINames.X303_2_Out_Shuttle_Turn_Ccw_Cyl_Sensor))
                    {
                        ...
                        #region 폭조절
                        stepAxis = FuncInline.enumLetsAxis.ST01_OutShuttle_Width;
                        motion_index = (int)((int)stepAxis / 2);
                        // 홈도 언클램프도 아니면 호밍
                        if (!GlobalVar.LetsAxisStatus[(int)stepAxis].isHomed &&
                            GlobalVar.LetsAxisStatus[(int)stepAxis].StandStill)
                    ...
                    DIO.WriteDOData(FuncInline.enumDONames.Y300_2_Out_Shuttle_CONTACT_STOPPER_Out_SOL, false);
                    DIO.WriteDOData(FuncInline.enumDONames.Y302_1_Out_Shuttle_CONTACT_STOPPER_In_SOL, false);
        ...
        public static bool CheckNearWidth(FuncInline.enumLetsAxis axis) // 폭조절 스텝모터가 해당 폭 위치에 있는가?
        {
            double teachingWidth = FuncInline.TeachingWidth[(int)axis];
            double actualWidth = GlobalVar.LetsAxisStatus[(int)axis].Position;
        ...
        public static bool InitAllWidth()
        {
            ...
            try
            {
                while (watch.ElapsedMilliseconds < 60 * 1000)
                {
                    if (GlobalVar.E_Stop ||
                        GlobalVar.DoorOpen)
                    {
                        FuncMotion.MoveStop((int)FuncInline.enumServoAxis.SV03_Rack1_Width);
                        FuncMotion.MoveStop((int)FuncInline.enumServoAxis.SV05_Rack2_Width);

                        FuncLetsMotion.Stop((int)FuncInline.enumLetsAxis.ST00_InShuttle_Width);
                        FuncLetsMotion.Stop((int)FuncInline.enumLetsAxis.ST01_OutShuttle_Width);
                        FuncLetsMotion.Stop((int)FuncInline.enumLetsAxis.ST02_OutConveyor_Width);
                        FuncLetsMotion.Stop((int)FuncInline.enumLetsAxis.ST03_InConveyor_Width);
                        if (FuncInline.InlineType >= FuncInline.enumInlineType.Gen5)
                        {
                            FuncLetsMotion.Stop((int)FuncInline.enumLetsAxis.ST04_NGBuffer);
                        }
                    ...
                    for (int i = 0; i < FuncInline.WidthStarted.Length; i++)
                    {
                        if (i == 4 && FuncInline.InlineType < FuncInline.enumInlineType.Gen5)
                        {
                            continue;
                        }

                        if (FuncInline.TeachingWidth[i] + FuncInline.OffsetWidth[i] < FuncInline.MinWidth ||
                            FuncInline.TeachingWidth[i] + FuncInline.OffsetWidth[i] > FuncInline.MaxWidth)
                        {
                            FuncWin.TopMessageBox("Width move failed. Check Teaching and Model Info.");
                            return false;
                        }
                        // 스텝모터
                        if (i < 5)
                        {
                            FuncInline.enumLetsAxis pmcAxis = (FuncInline.enumLetsAxis)i;

                            if (!GlobalVar.LetsAxisStatus[i].StandStill)
                            {
                                allInited = false;
                            }
                            else if (GlobalVar.LetsAxisStatus[i].StandStill &&
                                !CheckNearWidth(pmcAxis))
                            {
                                //debug(pmcAxis.ToString() + " 폭 조절 1");
                                FuncLetsMotion.ABSMove((int)pmcAxis, FuncInline.TeachingWidth[i], GlobalVar.WidthSpeed);
                                FuncInline.WidthStarted[i] = true;
                                allInited = false;
                                Thread.Sleep(100);
                            }
                            else if (!FuncInline.WidthDone[i] &&
                                GlobalVar.LetsAxisStatus[i].StandStill)
                            {
                        ...
                        {
                            FuncInline.enumServoAxis servoAxis = FuncInline.enumServoAxis.SV03_Rack1_Width;
                            if (i == 5)
                            {
                            ...
                            // 현재폭
                            double currentPos = GlobalVar.AxisStatus[(int)servoAxis].Position;  //기본 폭 250mm 추가한 현재 위치값
                            // 티칭폭
                            double teachingPos = FuncInline.TeachingWidth[i] + FuncInline.OffsetWidth[i];
                            //if (!GlobalVar.AxisStatus[(int)servoAxis].Homing &&
                            //    GlobalVar.AxisStatus[(int)servoAxis].Velocity <= 100 &&
                            //    GlobalVar.AxisStatus[(int)servoAxis].Velocity > 0)
                            //{
                            //    FuncMotion.MoveStop((int)servoAxis);
                            //}
                            if(!FuncInlineMove.IsArrived((int)servoAxis, FuncInline.TeachingWidth[i]) &&
                                GlobalVar.AxisStatus[(int)servoAxis].isHomed &&
                                GlobalVar.AxisStatus[(int)servoAxis].StandStill)
                            {
                                FuncInlineMove.MoveAbsolute((uint)servoAxis, FuncInline.TeachingWidth[i]);
                                FuncInline.WidthStarted[i] = true;
                                FuncInline.WidthDone[i] = false;
                                allInited = false;
                                Thread.Sleep(300);
                            }
                            else if(GlobalVar.AxisStatus[(int)servoAxis].isHomed &&
                                FuncInlineMove.IsArrived((int)servoAxis, FuncInline.TeachingWidth[i]))
                            {
                            ...
                            //    !FuncInlineMove.IsArrived((int)servoAxis, FuncInline.TeachingWidth[i] + FuncInline.OffsetWidth[i])) // 홈 위치면
                            //{
                            //    FuncInlineMove.MoveAbsolute((uint)servoAxis, FuncInline.TeachingWidth[i] + FuncInline.OffsetWidth[i]);
