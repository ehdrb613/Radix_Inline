        ...
        public bool SendStart(int site) // site 내 모든 어레이에 테스트 시작 지령 전송. 0부터 site1, site8, site15
        {
            ...
            try
            {
                ...
                if (FuncInline.InlineType >= FuncInline.enumInlineType.Gen6)
                {
                        ...
                        for (int i = 0; i < FuncInline.MaxArrayCount; i++)
                        {
                                ...
                                !FuncInline.PCBInfo[(int)FuncInline.enumTeachingPos.Site1_F_DT1 + site].SMDReady[i]) // ready 아니면
                            {
                                ...
                                if (!FuncInline.PCBInfo[(int)FuncInline.enumTeachingPos.Site1_F_DT1 + site].SMDReadySent[i])
                                {
                                    ...
                                    FuncLog.WriteLog_Tester($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()}.SMDStatus { (i + 1)} = FuncInline.enumSMDStatus.Ready_Sent");
                                    FuncInline.InsertCommLog(site + 1, i + 1, "RD", msg, 0, -1, "");
                                    FuncLog.WriteLog($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()}.SMDStatus { (i + 1)} = FuncInline.enumSMDStatus.Ready_Sent");
                                    SendMessage(msg);
                                    ...
                                    FuncInline.PCBInfo[(int)FuncInline.enumTeachingPos.Site1_F_DT1 + site].TestWatch[i].ElapsedMilliseconds > 10 * 1000)
                                {
                                    ...
                                    FuncLog.WriteLog($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()}.SMDStatus { (i + 1)}. TimeOut");
                                }
                            }
                                ...
                                FuncInline.PCBInfo[(int)FuncInline.enumTeachingPos.Site1_F_DT1 + site].ErrorCode[i] == 996)
                            {
                                ...
                                FuncLog.WriteLog($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()}.SMDStatus { (i + 1)}. Command NG");
                                all_ready = false;
                    ...
                    {
                        FuncLog.WriteLog($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()} 모두 OK 안됨");
                        return true;
                    }
                    #endregion
                    ...
                    for (int i = 0; i < FuncInline.MaxArrayCount; i++)
                    {
                            ...
                            FuncInline.PCBInfo[(int)FuncInline.enumTeachingPos.Site1_F_DT1 + site].Barcode[i].Length > 0) // Xout 에서 설정된 대로 코드가 등록된 것만 전송
                        {
                                ...
                                FuncInline.TestErrorRetestType[errorCode] != null)
                            {
                                ...
                                FuncLog.WriteLog($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()}.SMDStatus { (i + 1)} ST - {msg}");
                            }

                            if (errorCode >= 0 &&
                                FuncInline.TestErrorRetestType[errorCode] == null)
                            {
                                FuncLog.WriteLog($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()}.SMDStatus { (i + 1)}. 등록된 코드 없음");
                            ...
                            else
                            {
                                ...
                                {
                                    FuncLog.WriteLog_Tester($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()}.SMDStatus {(i + 1)} = FuncInline.enumSMDStatus.Command_Sent");
                                    FuncLog.WriteLog($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()}.SMDStatus {(i + 1)} = FuncInline.enumSMDStatus.Command_Sent");
                                }
                                all_sent = false;
                                //FuncInline.PCBInfo[(int)FuncInline.enumTeachingPos.Site1_F_DT1 + site].CommandRetry[i]++;
                                ...
                                FuncLog.WriteLog($"PCBInfo {(FuncInline.enumTeachingPos.Site1_F_DT1 + site).ToString()}.SMDStatus Send - {msg}");
                            }
                        }
                    ...
                    for (int i = 0; i < FuncInline.MaxArrayCount; i++)
                    {
                        ...
                        if (target && !info.SMDReady[i])
                        {
                            ...
                            if (!needSend &&
                                ...
                                info.TestWatch[i].ElapsedMilliseconds > 10000)
                            {
                ...
                for (int i = 0; i < FuncInline.MaxArrayCount; i++)
                {
                        ...
                        (info.SMDStatus[i] == FuncInline.enumSMDStatus.No_Test || info.Xout[i]))
                    {
                        if (info.Xout[i]) 
                        { 
                            info.ErrorCode[i] = 994; 
                            info.SMDStatus[i] = FuncInline.enumSMDStatus.Test_Fail;
                        }
                        continue;
                    }
                ...
                if (allSent)
                {
                        ...
                        info.PCBStatus != FuncInline.enumSMDStatus.Command_Sent)
                    {
                        ...
                        FuncLog.WriteLog($"AutoInline_PCBInfo{pos.ToString()}.PCBStatus = FuncInline.enumSMDStatus.Command_Sent");
                        info.PCBStatus = FuncInline.enumSMDStatus.Command_Sent;
                    ...
                    for (int i = 0; i < lines.Length - 1; i++)
                    {
                        ...
                        try
                        {
                            ...
                            if (this.IsDLPort)
                            {
                                ...
                                {
                                    FuncLog.WriteLog_Tester($"{SMD.PortName} DL site Parse missing. Drop: {line}");
                                    continue;
                                }
                            ...
                            else
                            {
                                ...
                                {
                                    FuncLog.WriteLog_Tester($"{SMD.PortName} FT site Parse missing. Drop: {line}");
                                    continue;
                                }
