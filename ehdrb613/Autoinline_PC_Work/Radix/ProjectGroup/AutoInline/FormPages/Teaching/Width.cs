...
namespace Radix.Popup.Teaching
{
    ...
    public partial class Width : Form
    {
        ...
        private FuncInline.enumLetsAxis activeStepAxis = FuncInline.enumLetsAxis.ST00_InShuttle_Width; // 스텝모터일 경우 순번
        ...
        private double[] teachingWidth = new double[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2];
        private double[] offsetWidth = new double[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2];
                    ...
                    this.InvokeRequired)
                {
                    this.Invoke(new MethodInvoker(delegate ()
                    {
                        ...
                        if (teachingIndex < Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length) // 스텝모터
                        {
                            ...
                            lblCurrentWidth.Text = (GlobalVar.LetsAxisStatus[(int)activeStepAxis].Position).ToString("F2");
                        }
                        else // 서보모터
                        {
                            if (debugCount >= 5)
                            ...
                            lblCurrentWidth.Text = (GlobalVar.AxisStatus[(int)activeServoAxis].Position).ToString("F2");
                        ...
                        #region 티칭값 표시
                        lblTeachingInConveyor.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST03_InConveyor_Width].ToString("F2");
                        lblTeachingInShuttle.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST00_InShuttle_Width].ToString("F2");
                        lblTeachingOutShuttle.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST01_OutShuttle_Width].ToString("F2");
                        lblTeachingOutConveyor.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST02_OutConveyor_Width].ToString("F2");
                        lblTeachingNGConveyor.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST04_NGBuffer].ToString("F2");
                        ...
                        #region 서보 상태표기
                        lblNot.Visible = GlobalVar.AxisStatus[(int)activeServoAxis].LimitSwitchNeg;
                        lblHome.Visible = GlobalVar.AxisStatus[(int)activeServoAxis].HomeAbsSwitch;
                        lblPot.Visible = GlobalVar.AxisStatus[(int)activeServoAxis].LimitSwitchPos;
                        lblHomeDone.Visible = activeServo ? GlobalVar.AxisStatus[(int)activeServoAxis].isHomed : GlobalVar.LetsAxisStatus[(int)activeStepAxis].isHomed;
                        #endregion
                ...
                if (jogWidth) //
                {
                        ...
                        !jogWidthUp)
                    {
                        if (GlobalVar.E_Stop ||
                            ...
                            (!activeServo && GlobalVar.LetsAxisStatus[(int)activeStepAxis].Errored))
                        {
                            if (activeServo)
                            {
                        ...
                        else if (!activeServo && Math.Abs(GlobalVar.LetsAxisStatus[(int)activeStepAxis].Velocity) > 0.1)
                        {
                            FuncLetsMotion.Stop((int)activeStepAxis);
                        }
                        else
        ...
        public void LoadAllValue()
        {
            try
            {
                ...
                {
                    teachingWidth[i] = FuncInline.PCBWidth + FuncInline.OffsetWidth[i];
                    FuncInline.TeachingWidth[i] = FuncInline.PCBWidth + FuncInline.OffsetWidth[i];
                }
        ...
        public void ApplyAllValue()
        {
            try
            {
                for (int i = 0; i < FuncInline.OffsetWidth.Length; i++)
                {
                    ...
                    FuncInline.OffsetWidth[i] = teachingWidth[i] - FuncInline.PCBWidth;
        ...
        private void pbWIdthMove_Click(object sender, EventArgs e)
        {
            ...
            {
                FuncLetsMotion.ABSMove((int)activeStepAxis, (double)numWidth.Value, (double)numSpeed.Value);
            ...
            {
                FuncInlineMove.MoveAbsolute((uint)activeServoAxis, (double)numWidth.Value, (double)numSpeed.Value);
        ...
        private void pbWidthNarrow_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (btnPitch.BackColor == Color.Lime)
            {
                if (teachingIndex < Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length)
                {
                    FuncLetsMotion.ABSMove((int)activeStepAxis, GlobalVar.LetsAxisStatus[(int)activeStepAxis].Position - (double)numPitch.Value, 10);
                    //PMCClass.ABSMove(activeStepAxis, 
                    //GlobalVar.LetsAxisStatus[(int)activeStepAxis].Position - (double)numPitch.Value, 
                    //(double)numSpeed.Value);
                }
                else
                {

                    FuncInlineMove.MoveAbsolute((uint)activeServoAxis,
            ...
            else
            {
                ...
                else
                {
                    ...
                    FuncInlineMove.MoveRelative((uint)activeServoAxis,
                                  (double)-500,
                                    (double)numSpeed.Value);
                }
            }
        ...
        private void SetSpeed()
        {
            ...
            {
                int index = -1;
                Array array = Enum.GetValues(typeof(FuncInline.enumLetsAxis));
                for (int i = 0; i < array.Length; i++)
                {
                    if (activeStepAxis == (FuncInline.enumLetsAxis)i)
                    {
                        index = i;
                        break;
                    }
                ...
                if (index >= 0)
                {
                    FuncInline.enumLetsAxis axis = (FuncInline.enumLetsAxis)index;
        ...
        private void btnPosClick(object sender, EventArgs e)
        {
            ...
            switch (sender_name)
            {
                case "btnPosInConveyor":
                    ...
                    numWidth.Value = (decimal)teachingWidth[(int)FuncInline.enumLetsAxis.ST03_InConveyor_Width];
                    lblTeachingWidth.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST03_InConveyor_Width].ToString("F2");
                    numOffsetWidth.Value = (decimal)FuncInline.OffsetWidth[(int)FuncInline.enumLetsAxis.ST03_InConveyor_Width];
                    activeStepAxis = FuncInline.enumLetsAxis.ST03_InConveyor_Width;
                    activeServo = false; // 축이 서보로 지정되어 있는가?
                    teachingIndex = (int)FuncInline.enumLetsAxis.ST03_InConveyor_Width; // 티칭이 서보/스텝 섞여 있어 변수로 지정한다.
                    break;
                case "btnPosInShuttle":
                    ...
                    numWidth.Value = (decimal)teachingWidth[(int)FuncInline.enumLetsAxis.ST00_InShuttle_Width];
                    lblTeachingWidth.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST00_InShuttle_Width].ToString("F2");
                    numOffsetWidth.Value = (decimal)FuncInline.OffsetWidth[(int)FuncInline.enumLetsAxis.ST00_InShuttle_Width];
                    activeStepAxis = FuncInline.enumLetsAxis.ST00_InShuttle_Width;
                    activeServo = false; // 축이 서보로 지정되어 있는가?
                    teachingIndex = (int)FuncInline.enumLetsAxis.ST00_InShuttle_Width; // 티칭이 서보/스텝 섞여 있어 변수로 지정한다.
                    break;
                case "btnPosOutShuttle":
                    ...
                    numWidth.Value = (decimal)teachingWidth[(int)FuncInline.enumLetsAxis.ST01_OutShuttle_Width];
                    lblTeachingWidth.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST01_OutShuttle_Width].ToString("F2");
                    numOffsetWidth.Value = (decimal)FuncInline.OffsetWidth[(int)FuncInline.enumLetsAxis.ST01_OutShuttle_Width];
                    activeStepAxis = FuncInline.enumLetsAxis.ST01_OutShuttle_Width;
                    activeServo = false; // 축이 서보로 지정되어 있는가?
                    teachingIndex = (int)FuncInline.enumLetsAxis.ST01_OutShuttle_Width; // 티칭이 서보/스텝 섞여 있어 변수로 지정한다.
                    break;
                case "btnPosOutConveyor":
                    ...
                    numWidth.Value = (decimal)teachingWidth[(int)FuncInline.enumLetsAxis.ST02_OutConveyor_Width];
                    lblTeachingWidth.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST02_OutConveyor_Width].ToString("F2");
                    numOffsetWidth.Value = (decimal)FuncInline.OffsetWidth[(int)FuncInline.enumLetsAxis.ST02_OutConveyor_Width];
                    activeStepAxis = FuncInline.enumLetsAxis.ST02_OutConveyor_Width;
                    activeServo = false; // 축이 서보로 지정되어 있는가?
                    teachingIndex = (int)FuncInline.enumLetsAxis.ST02_OutConveyor_Width; // 티칭이 서보/스텝 섞여 있어 변수로 지정한다.
                    break;
                case "btnPosNGConveyor":
                    ...
                    numWidth.Value = (decimal)teachingWidth[(int)FuncInline.enumLetsAxis.ST04_NGBuffer];
                    lblTeachingWidth.Text = teachingWidth[(int)FuncInline.enumLetsAxis.ST04_NGBuffer].ToString("F2");
                    numOffsetWidth.Value = (decimal)FuncInline.OffsetWidth[(int)FuncInline.enumLetsAxis.ST04_NGBuffer];
                    activeStepAxis = FuncInline.enumLetsAxis.ST04_NGBuffer;
                    activeServo = false; // 축이 서보로 지정되어 있는가?
                    teachingIndex = (int)FuncInline.enumLetsAxis.ST04_NGBuffer; // 티칭이 서보/스텝 섞여 있어 변수로 지정한다.
                    break;
                case "btnPosRack1":
                    btnPosInConveyor.BackColor = Color.White;
                    btnPosInShuttle.BackColor = Color.White;
