    ...
    public partial class LiftPosition : Form
    {
        ...
        private void TimerUI(Object state) // 화면 제어 쓰레드 타이머 함수
        {
            ...
            try
            {
                    ...
                    this.Invoke(new MethodInvoker(delegate ()
                {
                    ...
                    #region 서보 상태표기
                    lblNot.Visible = GlobalVar.AxisStatus[(int)actionAxis].LimitSwitchNeg;
                    lblHome.Visible = GlobalVar.AxisStatus[(int)actionAxis].HomeAbsSwitch;
                    lblPot.Visible = GlobalVar.AxisStatus[(int)actionAxis].LimitSwitchPos;
                    lblHomeDone.Visible = GlobalVar.AxisStatus[(int)actionAxis].isHomed;
                    #endregion
                    ...
                    {
                        foreach (Control conPos in groupBox1.Controls) // 사이트 묶음 전체
                        {
                        ...
                        foreach (Control conPos in groupBox2.Controls) // 사이트 묶음 전체
                        {
                            if (conPos.GetType() == typeof(Label))
                            {
                                Label label = (Label)conPos;
                        ...
                        foreach (Control conPos in groupBox3.Controls) // 사이트 묶음 전체
                        {
                            if (conPos.GetType() == typeof(Label))
                            {
                                Label label = (Label)conPos;
                                if (label.Name.Contains("lblShuttleTeaching"))
                                {
                                    string str = label.Name.Replace("lblShuttleTeaching", "");
                                    string[] idxs = str.Split('_');
                                    if (idxs.Length == 2)
                                    {
                                        int liftIndex = -1;
                                        ...
                                        if (liftIndex >= 0 &&
                                            posIndex >= 0)
                                        {
                                            label.Text = ShuttlePos[liftIndex, posIndex].ToString("F2");
                                        }
                                    }
                                }
                            }
                        }
                        foreach (Control conPos in groupBox4.Controls) // 사이트 묶음 전체
                        {
                            if (conPos.GetType() == typeof(Label))
                            {
                                Label label = (Label)conPos;
        ...
        public void LoadAllValue()
        {
            try
            {
                ...
                if (activeNum < 2)
                {
                    lblTeaching.Text = liftPos[activeNum, (int)activeLiftPos].ToString("F2");
                }
                else
                {
        ...
        private void pbRobotApply_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (activeNum < 2)
                {
                    liftPos[activeNum, (int)activeLiftPos] = double.Parse(lblPos.Text);
                    lblTeaching.Text = liftPos[activeNum, (int)activeLiftPos].ToString("F2");

                    FuncLog.WriteLog("Lift Pos apply : " + activeLiftPos.ToString() + " - " + liftPos[activeNum, (int)activeLiftPos]);
                }
                else
                {
                    ...
                    FuncLog.WriteLog("Shuttle Pos apply : " + activeShuttlePos.ToString() + " - " + ShuttlePos[activeNum - 2, (int)activeShuttlePos]);
                }
                ...
                //        FuncLog.WriteLog("Lift Pos apply : " + pos.ToString() + " - " + liftPos[activeLift, (int)pos]);
                //    }
                //}
                #endregion
        ...
        {
            numPos.Value = (decimal)liftPos[0, (int)activeLiftPos];
            //debug("shown");
            ...
            try
            {
                    ...
                    name.Contains("btnPosOutShuttleDown"))
                {
                    ...
                    name.Contains("btnPosRearNGLine") ||
                     name.Contains("btnPosRearInshuttlePos"))
                {
                    isLift = true;
                    //서보 RearLift 자동 선택
                ...
                if (validPos)
                {
                    ...
                    {
                        lblTeaching.Text = liftPos[activeNum, (int)activeLiftPos].ToString("F2");
                        numPos.Value = (decimal)liftPos[activeNum, (int)activeLiftPos];
