        ...
        private void btnUseSite_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!siteOnly)
                {
                    ...
                    {
                        int smdIndex = FuncInline.MapSmdIndex(((int)FuncInline.DetailSite - (int)FuncInline.enumTeachingPos.Site1_F_DT1));    //세대별 변환
                        
                        int pcIndex = smdIndex;
            ...
            try
            {
                    ...
                    GlobalVar.SystemStatus == enumSystemStatus.ErrorStop)
                {
                    ...
                    for (FuncInline.enumTeachingPos pos = startPos; pos <= endPos; pos++)
                    {
                        ...
                        // ───────────────── 사이트(1~26): SiteIoMaps 사용
                        if (pos >= FuncInline.enumTeachingPos.Site1_F_DT1 &&
                            pos <= FuncInline.enumTeachingPos.Site26_R_FT3)
                        {
                            // 사이트 CCW 구동으로 토출 (맵핑된 DO만 사용)
                            if (FuncInline.SiteIoMaps.TryGetSiteMotor(pos, out var siteCwDo, out var siteCcwDo))
                                DIO.WriteDOData(siteCcwDo, true);
                        ...
                        // ───────────────── 비-사이트 설비 (최신 DO 명칭으로 정리)
                        switch (pos)
                        {
                            case FuncInline.enumTeachingPos.InConveyor:
                                // 인셔틀 컨베이어 정/역 전환형: CW만 ON, CCW는 OFF
                            ...
                            case FuncInline.enumTeachingPos.FrontScanSite:
                                ...
                                //Scan중이면 스캔 액션도 초기화
                                if (FuncInline.ScanAction == FuncInline.enumScanAction.FrontScan)
                                {
                            ...
                            case FuncInline.enumTeachingPos.RearPassLine:
                                DIO.WriteDOData(FuncInline.enumDONames.Y305_4_Rear_PassLine_Motor_Cw, true);
                                FuncInline.RearPassLineAction = FuncInline.enumLiftAction.Waiting;
                                break;

                            case FuncInline.enumTeachingPos.RearNGLine:
                                DIO.WriteDOData(FuncInline.enumDONames.Y404_0_Rear_NgLine_Motor_Cw, true);
                            ...
                            case FuncInline.enumTeachingPos.Lift1_Down:
                                ...
                                if (FuncInline.Lift1Action.ToString().Contains("Down"))
                                    AutoInline.Class.FrontRack.Action = FrontRack.enumAction.Waiting;
                                break;
                            ...
                            case FuncInline.enumTeachingPos.Lift2_Up:
                                ...
                                //Scan중이면 스캔 액션도 초기화
                                if(FuncInlineMove.CheckNearLiftPos(FuncInline.enumLiftName.RearLift, FuncInline.enumLiftPos.RearScanPos) &&
                                    FuncInline.ScanAction == FuncInline.enumScanAction.RearScan)
                                {
                                    FuncInline.ScanAction = FuncInline.enumScanAction.Waiting;
                                } 
                                break;
                            ...
                            case FuncInline.enumTeachingPos.Lift2_Down:
                                ...
                                if (FuncInline.Lift2Action.ToString().Contains("Down"))
                                    FuncInline.Lift2Action = FuncInline.enumLiftAction.Waiting;
                                break;
                    ...
                    while (clearQueue.Count > 0)
                    {
                        ...
                        {
                            // 공통: 정보 클리어 및 액션 Idle
                            FuncInline.enumTeachingPos pos = FuncInline.enumTeachingPos.Site1_F_DT1 + (int)siteIndex ;

                            // 사이트 CCW 구동으로 토출 (맵핑된 DO만 사용)
                            if (FuncInline.SiteIoMaps.TryGetSiteMotor(pos, out var siteCwDo, out var siteCcwDo))
                            {
                                DIO.WriteDOData(siteCwDo, false);
                                DIO.WriteDOData(siteCcwDo, false);
                            }
