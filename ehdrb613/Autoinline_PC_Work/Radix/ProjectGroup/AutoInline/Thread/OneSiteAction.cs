...
namespace Radix
{
    ...
    class OneSiteAction
    {
        ...
        public OneSiteAction(int siteIndex)
        {
            ...
            pcIndex = FuncInline.MapSmdIndex(SiteIndex);
        ...
        public void Run()
        {
            ...
            while (!GlobalVar.GlobalStop)
            {
                try
                {
                    ...
                    // 코드 가독성을 위한 단축 참조
                    var pcbInfo = FuncInline.PCBInfo[(int)pos];

                    // =================================================================
                    // [메인 로직] AutoRun 상태이고 에러가 없을 때만 동작
                    // =================================================================
                        ...
                        GlobalVar.SystemStatus != enumSystemStatus.ErrorStop)
                    {
                        ...
                        {
                            // 상태별 동작 (Switch-Case 구조로 변경)
                            switch (FuncInline.SiteAction[SiteIndex])
                            {
                                // =============================================================
                                // 1. Loading : 제품 투입
                                // =============================================================
                                ...
                                // =============================================================
                                // 2. Testing : 검사 수행 (Start전송 -> 검사 -> 타임아웃체크)
                                // =============================================================
                                case FuncInline.enumSiteAction.Testing:
                                    #region Testing

                                    // 2-0. 타이머 관리
                                    if (pcbInfo.StopWatch == null || !pcbInfo.StopWatch.IsRunning)
                                        Util.StartWatch(ref pcbInfo.StopWatch);

                                    // 2-1. 검사 시작 (Before_Command -> Send Start)
                                    if (pcbInfo.PCBStatus == FuncInline.enumSMDStatus.Before_Command)
                                    {
                                        // 핀 다운 확인 (물리적 고정 확인)
                                        ...
                                        {
                                            // [통신] Start Command 전송
                                            // 개별 어레이 상태 변경은 응답을 받았을 때 처리되거나, 여기서 미리 Command_Sent로 변경
                                            FuncInline.ComSMD[smdIndex].SendStart(SiteIndex);

                                            // 전체 상태 변경
                                            pcbInfo.PCBStatus = FuncInline.enumSMDStatus.Command_Sent;
                                        }
                                        else
                                        {
                                    ...
                                    // 2-2. 검사 진행 모니터링 (Loop)
                                    bool isTestingDone = CheckTestingProcess(SiteIndex, pos, smdIndex);

                                    // 2-3. 검사 완료 처리
                                    if (isTestingDone)
                                    {
                                        // 테스트 시간 기록
                                        if (pcbInfo.StopWatch.IsRunning)
                                        {
                                ...
                                // =============================================================
                                // 3. UnLoading : 배출 대기 (핀업 후 Rack이 가져가길 기다림)
                                // =============================================================
                                case FuncInline.enumSiteAction.Unloading:
                                    #region UnLoading
                                    // 1. 물리적 해제 (핀 업 & 언클램프)
                                    ...
                                    if(SiteIndex < 13)
                                    {
                                        ...
                                        // Site의 PCBStatus를 UnKnown으로 초기화합니다.
                                ...
                                // 4. Waiting : 대기
                                // =============================================================
                                case FuncInline.enumSiteAction.Waiting:
                                    // 특별한 동작 없음. 로봇/셔틀이 로딩 명령을 줄 때까지 대기
                                    break;

                                // =============================================================
                                // 5. ReOpen / ReClose : 재검사 시 핀 동작
                                // =============================================================
                                case FuncInline.enumSiteAction.ReOpen:
                                    if (FuncInline.SiteReOpen(pos)) // 핀 업
                                    {
                                        ChangeSiteAction(SiteIndex, FuncInline.enumSiteAction.ReClose, "ReOpen Done");
                                        Util.ResetWatch(ref FuncInline.SiteCheckTime[SiteIndex]);
                                    }
                                    break;

                                case FuncInline.enumSiteAction.ReClose:
                                    if (FuncInlineAction.SitePCBLoad(pos)) // 핀 다운
                                    {
                                        ChangeSiteAction(SiteIndex, FuncInline.enumSiteAction.Testing, "ReClose Done -> Testing");
                                        Util.ResetWatch(ref FuncInline.SiteCheckTime[SiteIndex]);

                                        // 타이머 재시작
                                        Util.StartWatch(ref pcbInfo.StopWatch);
                                        for (int k = 0; k < pcbInfo.TestWatch.Length; k++)
                                            Util.StartWatch(ref pcbInfo.TestWatch[k]);
                                    }
                                    break;
                            } // End Switch
                        }
                    ...
                    // 사이트 사용 유무 상태 업데이트
                    FuncInline.SiteState[SiteIndex] = FuncInline.UseSite[SiteIndex] ?
                        FuncInline.enumSiteState.Valid : FuncInline.enumSiteState.NotUse;
        ...
        /// 사이트 액션 변경 및 로그 기록
        /// </summary>
        private void ChangeSiteAction(int siteIdx, FuncInline.enumSiteAction newAction, string reason)
        {
            if (FuncInline.SiteAction[siteIdx] != newAction)
            {
                FuncLog.WriteLog_Tester($"Site{siteIdx + 1} Action Change: {FuncInline.SiteAction[siteIdx]} ==> {newAction} ({reason})");
                FuncInline.SiteAction[siteIdx] = newAction;
            }
        }

        /// <summary>
        /// DL/FT 구분에 따른 검사 대상 어레이 확인
        /// </summary>
        private bool IsTargetArray(FuncInline.enumTeachingPos pos, int arrayIdx)
        {
            // 1. 바코드가 있는 경우 (Gen6 통합 or Gen5 FT)
            if (FuncInline.PCBInfo[(int)pos].Barcode[arrayIdx].Length > 0)
                return true;

            // 2. 바코드가 없고 Gen5 이하인 경우 (Gen5 DL Test)
            if (FuncInline.InlineType < FuncInline.enumInlineType.Gen6 &&
                FuncInline.ArrayUse[arrayIdx])
                return true;
        ...
        /// <summary>
        /// 검사 진행 상태 체크, 타임아웃 처리, 결과 집계 (Testing 상태에서 호출)
        /// </summary>
        /// <returns>검사가 모두 완료되었으면 true</returns>
        private bool CheckTestingProcess(int siteIdx, FuncInline.enumTeachingPos pos, int smdIndex)
        {
            var pcbInfo = FuncInline.PCBInfo[(int)pos];
            bool hasTesting = false;
            bool hasTimeout = false;
            bool allFinished = true; // 모든 대상이 결과(Pass/Fail/NoTest)가 나왔는지

            // 1. 어레이별 상태 루프
            for (int j = 0; j < FuncInline.MaxArrayCount; j++)
            {
                if (!IsTargetArray(pos, j)) continue; // 대상 아니면 스킵

                // 타이머 안전장치
                if (pcbInfo.TestWatch[j] == null || !pcbInfo.TestWatch[j].IsRunning)
                    Util.StartWatch(ref pcbInfo.TestWatch[j]);

                // 타임아웃 체크 (Testing or Command_Sent 상태일 때)
                if ((pcbInfo.SMDStatus[j] == FuncInline.enumSMDStatus.Testing ||
                     pcbInfo.SMDStatus[j] == FuncInline.enumSMDStatus.Command_Sent) &&
                    pcbInfo.TestWatch[j].ElapsedMilliseconds > FuncInline.TestTimeout * 1000)
                {
                    // 타임아웃 처리
                    pcbInfo.TestWatch[j].Stop();
                    pcbInfo.SMDStatus[j] = FuncInline.enumSMDStatus.Test_Timeout;
                    pcbInfo.ErrorCode[j] = 999;
                    FuncInline.ArrayNGCount[siteIdx, j]++;

                    // 불량 누적/비율 Block 로직 (즉시 차단)
                    if (FuncInline.BlockDefectArray &&
                        (FuncInline.ArrayNGCount[siteIdx, j] >= FuncInline.BlockNGCount ||
                         FuncInline.CheckDefectRateBlock(siteIdx + 1, j + 1)))
                    {
                        FuncInline.UseSite[siteIdx] = false;
                        FuncLog.WriteLog($"Site {siteIdx + 1} Blocked due to NG/Rate.");
                    }
            ...
            if (hasTimeout)
            {
                int localSiteIdx = (int)pos - (int)FuncInline.enumTeachingPos.Site1_F_DT1;
                FuncInline.ComSMD[smdIndex].SendStop(localSiteIdx);
                pcbInfo.PCBStatus = FuncInline.enumSMDStatus.Test_Timeout;
            }

            // 3. 전체 완료 여부 판단
            if (allFinished && !hasTesting)
            {
                // 최종 상태 결정 (Pass / Fail)
                bool anyFail = false;
                for (int j = 0; j < FuncInline.MaxArrayCount; j++)
        ...
        //            FuncInline.enumTeachingPos pos = FuncInline.enumTeachingPos.Site1_F_DT1 + SiteIndex;
        //            // ===== 자동운전 메인 =====
        //            if ((int)GlobalVar.SystemStatus >= (int)enumSystemStatus.AutoRun) // 에러상태가 아닐 때만 동작
        //            {
        //                // 패스모드가 아니면 사이트 로직 수행
        //                if (!FuncInline.PassMode)
        //                {
        ...
        //                        if (FuncInlineAction.OneSiteMove(pos,FuncInline.enumSiteAction.Loading)) // 완료 시 true
        //                        { 
        ...
        //                            if (FuncInline.SiteAction[SiteIndex] != FuncInline.enumSiteAction.Testing)
        //                            {
        //                                FuncLog.WriteLog_Tester($"Site{SiteIndex + 1} action change : {FuncInline.SiteAction[SiteIndex]} ==> Testing");
        //                            }
        //                            FuncInline.SiteAction[SiteIndex] = FuncInline.enumSiteAction.Testing;
        //                            Util.ResetWatch(ref FuncInline.SiteCheckTime[(int)pos - (int)FuncInline.enumTeachingPos.Site1_F_DT1]);
        ...
        //                            FuncInline.ComSMD[smdIndex].SendStart(SiteIndex); // 지령 전송
        ...
        //                        GlobalVar.SystemStatus != enumSystemStatus.ErrorStop &&
        //                        FuncInline.SiteAction[SiteIndex] == FuncInline.enumSiteAction.Testing &&
        //                        FuncInline.SiteIoMaps.TryGetPcbDockDI(pos, out var dockDi) && DIO.GetDIData(dockDi) &&
        ...
        //            if (!FuncInline.UseSite[SiteIndex]) // NotUse 지정
        //            {
