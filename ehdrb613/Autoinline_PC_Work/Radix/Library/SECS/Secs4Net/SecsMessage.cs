...
namespace Secs4Net {
    ...
    public sealed class SecsMessage : MarshalByRefObject, ISerializable {
        ...
        public SecsMessage(byte s, byte f, string name, bool replyExpected, Item item) {
            if (s > 0x7F)
                throw new ArgumentOutOfRangeException("s", s, "Stream number 는 127를 초과할 수 없음.");
        ...
        internal SecsMessage(byte s, byte f, string name, bool replyExpected, byte[] itemBytes, ref int index)
            : this(s, f, name, replyExpected, Decode(itemBytes, ref index)) { }
