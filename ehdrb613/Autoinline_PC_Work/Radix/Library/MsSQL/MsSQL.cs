    ...
    public class MsSQL
    {
        ...
        public bool Connect()
        {
            try
            {
                // 1) master로 접속해 DB 보장
                var csMaster =
                    $"Server={server},{port};Database=master;User ID={uid};Password={pwd};Connect Timeout=15;Encrypt=False;";
                using (var mcon = new SqlConnection(csMaster))
                {
                    mcon.Open();
...
END";
                    ...
                    // ONLINE 될 때까지 대기 (최대 120초)
                    using (var wait = mcon.CreateCommand())
                    {
                        wait.CommandText = @"
DECLARE @dbname sysname = @db;
WHILE DB_ID(@dbname) IS NULL OR
      (SELECT state_desc FROM sys.databases WHERE name=@dbname) <> 'ONLINE'
BEGIN
    WAITFOR DELAY '00:00:01';
END";
                        wait.Parameters.Add("@db", SqlDbType.NVarChar, 128).Value = db;
                        wait.CommandTimeout = 120;
                ...
                // 2) 대상 DB로 접속(유지)
                var csDb =
                    $"Server={server},{port};Database={db};User ID={uid};Password={pwd};Connect Timeout=15;Encrypt=False;";
                con = new SqlConnection(csDb);
                con.Open();

                connected = (con.State == ConnectionState.Open);
            ...
            {
                System.Diagnostics.Debug.WriteLine($"SQL {ex.Number}/{ex.State}: {ex.Message}");
                connected = false;
        ...
        public string[,] Read(string sql)
        {
            lock (lock_obj)     // DB 사용시 스레드 안전을 위해
            {
                try
                {
                    ...
                    while (reader.Read())
                    {
                        FuncLog.WriteLog("rowIndex : " + rowIndex);
                        for (int i = 0; i < reader.FieldCount; i++)
                        {
                            FuncLog.WriteLog(reader[i]);
                            string col = reader[i] as string;
                            FuncLog.WriteLog("rs " + rowIndex + "," + i + " : " + col);
                        }
                        rowIndex++;
                    }
                    //*/
