    ...
    class DIO
    {
        ...
        public static void WriteDIData(int di, bool value) // WriteDIData(DI코드, 값) (simulation용)DI 데이터를 쓴다. 
        {
            try
            {
                ...
                {
                    FuncLog.WriteLog("DI Write : " + ((FuncInline.enumDINames)di).ToString() + " - " + value.ToString());
                }
        ...
        public static void WriteDOData(int dn, bool data) // WriteDOData(DO코드, 값) DO 쓰기 
        {
            ...
            if (GlobalVar.Simulation &&
                ...
                GetDORead(dn) != data)
            {
                FuncLog.WriteLog("DO Write : " + ((FuncInline.enumDONames)dn).ToString() + " - " + data.ToString());
            }
        ...
        {
            FuncLog.WriteLog("메시지: " + message);
            FuncLog.WriteLog("메서드 이름: " + memberName);
            FuncLog.WriteLog("파일의 경로: " + sourceFilePath);
            FuncLog.WriteLog("줄 번호: " + sourceLineNumber);
        }
