    ...
    class FuncKeyboard
    {
        ...
        public static void CloseKeyboard() // 가상 키보드 닫기
        {
            try
            {
                ...
                // 프로세스 이름으로 모든 프로세스 가져오기 
                Process[] processes = Process.GetProcessesByName(processName);

                foreach (Process keyboardProcess in processes)
                {
                    try
                    {
                        // 메인 윈도우 핸들이 있는지 확인
                        if (keyboardProcess.MainWindowHandle != IntPtr.Zero)
                        {
                            // 윈도우 닫기 메시지 전송
                            FuncWin.PostMessage(keyboardProcess.MainWindowHandle, WM_SYSCOMMAND, (int)(new IntPtr(SC_CLOSE)), (int)(new IntPtr(0)));
                        }
                        ...
                        {
                            // 창 핸들이 없는 경우 강제 종료가 필요하다면 아래 주석 해제 (권한 에러 가능성 있음)
                            // keyboardProcess.Kill(); 
                        }
                    }
                    catch (System.ComponentModel.Win32Exception)
                    {
                        // 권한이 없는 시스템 프로세스 등을 건너뜀 (액세스 거부 예외 무시)
                        continue;
                    }
                    finally
                    {
