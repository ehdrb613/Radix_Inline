    ...
    public static class FuncError
    {
        ...
        public static bool CheckError(FuncInline.structError er) // CheckError(에러명) 에러 발생 여부 확인
        {
            try
            {
                ...
                //    }
                //}
                // SystemErrorQueue를 직접 순회합니다. by DG 260209
                foreach (var item in GlobalVar.SystemErrorQueue)
                {
                    FuncInline.structError errorItem = (FuncInline.structError)item;

                    if (errorItem.ErrorCode == er.ErrorCode &&
                        errorItem.ErrorPart == er.ErrorPart)
                    {
                        return true;
                    }
                }
