...
namespace Radix
{
    ...
    public static class FuncMotion
    {
        ...
        public static double ReCalcAxisPos(FuncInline.enumServoAxis axis, double pos) // 기구상의 실제 거리를모터 자체의 거리 계산값으로 환산
        {
            switch (axis)
            {
                ...
                case FuncInline.enumServoAxis.SV02_Lift1:
                    return pos;
                case FuncInline.enumServoAxis.SV03_Rack1_Width:
                    return - (pos - 250); //이동 방향 역방향이라 표기 변경
                case FuncInline.enumServoAxis.SV04_Lift2:
                    return pos;
                case FuncInline.enumServoAxis.SV05_Rack2_Width:
                    return - (pos - 250); //이동 방향 역방향이라 표기 변경
         ...
         */
        public static double GetRealPosition(int axis, double pos) // 프로그램 UI연결할 실제 좌표값 계산
        {
        ...
        */
        ...
        public static bool MoveHome(uint axis, bool WaitDone, uint Timeout) // MoveHome(축번호, 완료대기, 대기시간) 서보 Home 찾기 
        {
            ...
            switch (GlobalVar.MasterType)
            {
                ...
                case enumMasterType.AXL:
                    ...
                    dVelFirst = 200000; //파나소닉 세팅 해상도 100000
        ...
        {
            MoveAbsolute(axis, Pos, Vel, Vel * 5, Vel * 5, Vel * 25);
        }
         ...
         * @return void
         */
        public static void MoveAbsolute(uint axis, double Pos, double Vel, double Acc, double Dec, double Jerk) // MoveAbsolute(축번호, 좌표, 속도, 가속, 감속, 저크, 완료대기, 대기시간) 서보 절대위치 이동  
        {
            //debug("MoveAbsolute(" + axis.ToString() + "," + Pos.ToString() + "," + Vel.ToString() + "," + Acc.ToString() + "," + Dec.ToString() + "," + Jerk.ToString());
...
//*/
                ...
                case enumMasterType.AXL:
                    ...
                    //FuncLog.WriteLog("MoveAbsolute : " + axis + " - " + pos);
                    if (true)
                    {
        ...
        public static void MoveRelative(uint axis, double Pos, double Vel) // MoveAbsolute(축번호, 좌표, 속도) 서보 절대위치 이동  
        {
            ...
            MoveRelative(axis, Pos, Vel, Vel * 5, Vel * 5, Vel * 25);
        }
        /**
         * @brief 서보 상대위치 이동 
         *      지령은 실제 거리 또는 속도값으로 실행하고
         ...
         * @return void
         */
        public static void MoveRelative(uint axis, double Pos, double Vel, double Acc, double Dec, double Jerk) // MoveAbsolute(축번호, 좌표, 속도, 가속, 감속, 저크, 완료대기, 대기시간) 서보 절대위치 이동  
        {
         ...
         *      지령은 실제 속도값으로 실행하고
         *      동작시 pulse 환산하여 실제 지령을 서보드라이브에 전달한다.
         * @param axis_1 서보1 순번 축번호 낮은 순번으로
