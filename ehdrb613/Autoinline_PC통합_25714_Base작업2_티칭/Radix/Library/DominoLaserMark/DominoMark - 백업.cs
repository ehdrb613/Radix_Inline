...
namespace Radix
{
    public class DominoMark_backup
    {
        ...
        bool Init() // 접속 및 초기 동작부터 펌핑까지
        {
            ...
            while (!connected)
            {
                ...
                if (GlobalVar.TickCount64 - startTime > 10000)
                {
                    ...
                    debug("Domino Init() Connect failed!");
                    return false;
                }
                Thread.Sleep(100);
                Connect();
            ...
            while (_bThreadStateRun)
            {
                try
                {
                    ...
                    if (buffer[rbytes - 1] == 10)   // 수신 문자열 끝이 0x0A로 끝나면
                    {
                        ...
                        {
                            LaserState = 2;
                            //eventWait.Set();  // 프린트 중 신호는 무시하고 MSG 3 완료만 받아서 처리하자
                            debug("RECV: " + str);
                        }
        ...
        public bool WaitForJobResult(int timeoutMS)
        {
            ...
            {
                IsSignaled = eventWait.WaitOne(100);
                if (IsSignaled) break;
