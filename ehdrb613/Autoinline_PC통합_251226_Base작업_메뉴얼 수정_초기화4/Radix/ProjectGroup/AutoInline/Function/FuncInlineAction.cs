...
namespace Radix
{
    public class FuncInlineAction
    {
        ...
        public static bool CheckOriginDone(FuncInline.enumInitialize part)
        {
            ...
            switch (part)
            {
                ...
                case FuncInline.enumInitialize.NgBuffer:
                    return GlobalVar.LetsAxisStatus[(int)FuncInline.enumLetsAxis.ST04_NGBuffer].isHomed &&                                                     
        ...
        public static bool InitAllWidth()
        {
            ...
            try
            {
                while (watch.ElapsedMilliseconds < 90 * 1000)
                {
                    if (GlobalVar.E_Stop ||
                        GlobalVar.DoorOpen)
                    {
                    ...
                    for (int i = 0; i <  FuncInline.WidthStarted.Length; i++)
                    {
                        ...
                        if (FuncInline.TeachingWidth[i] + FuncInline.WidthOffset[i] < FuncInline.MinWidth ||
                            FuncInline.TeachingWidth[i] + FuncInline.WidthOffset[i] > FuncInline.MaxWidth)
                        {
                        ...
                        if (i < 5)
                        {
                                ...
                                !CheckNearWidth(pmcAxis))
                            {
                                ...
                                FuncLetsMotion.ABSMove((int)pmcAxis, FuncInline.TeachingWidth[i] + FuncInline.WidthOffset[i], GlobalVar.WidthSpeed);
                        ...
                        else
                        {
                            ...
                            // 티칭폭
                            double teachingPos = FuncInline.TeachingWidth[i] + FuncInline.WidthOffset[i];
                            ...
                            // 홈도 지정폭도 아니면 호밍 간다.
                            else if (!GlobalVar.AxisStatus[(int)servoAxis].Homing &&
                                !FuncInline.WidthDone[i] &&
                                !FuncMotion.CheckNearPos((double)0,
                                                    GlobalVar.AxisStatus[(int)servoAxis].Position,
                                                    10) && // 원점 아니고
                                !FuncMotion.CheckNearPos(FuncInline.TeachingWidth[i] + FuncInline.WidthOffset[i],
                                                    GlobalVar.AxisStatus[(int)servoAxis].Position,
                                                    0.1)) // 지정 위치도 아니고
                            {
                                //debug(servoAxis.ToString() + " 홈도 지정폭도 아니면 호밍 3. " + GlobalVar.AxisStatus[(int)servoAxis].Position);
                                FuncMotion.MoveHome((uint)servoAxis);
                                //FuncMotion.MoveAbsolute((uint)servoAxis,
                                //                    0,
                                //                    Util.GetRealSpeed((int)servoAxis, 20));
                                FuncInline.WidthStarted[i] = true;
                                FuncInline.WidthDone[i] = false;
                                allInited = false;
                            }
                            // 이외 원점 위치면 지정폭으로 이동
                                ...
                                !FuncInline.WidthDone[i] &&
                                FuncMotion.CheckNearPos((double)0,GlobalVar.AxisStatus[(int)servoAxis].Position,10)) // 홈 위치면
                            {
                                //debug(servoAxis.ToString() + " 이외 폭이 다르면 지정폭으로 이동 4");
                                FuncMotion.MoveAbsolute((uint)servoAxis,
                                    FuncMotion.ReCalcAxisPos((int)servoAxis,FuncInline.TeachingWidth[i] + FuncInline.WidthOffset[i]),   //실제값으로 변경후 이동
                                    20);
                                FuncInline.WidthStarted[i] = true;
                                FuncInline.WidthDone[i] = false;
                                allInited = false;
                            ...
                            // 초기화 완료
                                ...
                                !GlobalVar.AxisStatus[(int)servoAxis].Homing &&
                                GlobalVar.AxisStatus[(int)servoAxis].StandStill &&
                                FuncMotion.CheckNearPos(FuncInline.TeachingWidth[i] + FuncInline.WidthOffset[i],
                                GlobalVar.AxisStatus[(int)servoAxis].Position, 0.1))
                            {
                                //debug(servoAxis.ToString() + " 폭 초기화 완료 5");
                                FuncInline.WidthDone[i] = true;
                            }
                            else if (!FuncMotion.CheckNearPos(FuncInline.TeachingWidth[i] + FuncInline.WidthOffset[i],
                                        GlobalVar.AxisStatus[(int)servoAxis].Position,
                                                   0.1))
                            {
                                allInited = false;
