   ...
   */
    ...
    public class FuncInline
    {
        ...
        public static void InitAllSubAction()
        {
            ...
            ((AutoInline)GlobalVar.Class).InShuttle.Action = InShuttle.enumAction.Waiting;    //inshuttle 상태 변경
            ((AutoInline)GlobalVar.Class).FrontRack.Action = FrontRack.enumAction.Waiting;    //FrontRack 상태 변경
            ((AutoInline)GlobalVar.Class).RearRack.Action = RearRack.enumAction.Waiting;    //RearRack 상태 변경
            ((AutoInline)GlobalVar.Class).OutShuttle.OutShuttleAction = OutShuttle.OutShuttle_enumAction.Waiting;    //OutShuttle 상태 변경
            ((AutoInline)GlobalVar.Class).OutShuttle.OutConveyorAction = OutShuttle.OutConveyor_enumAction.Waiting;    //OutConveyor 상태 변경
            ((AutoInline)GlobalVar.Class).Scan.Action = Scan.enumAction.Waiting;    //Scan 상태 변경
        ...
        public static double[] WidthOffset = new double[Enum.GetValues(typeof(FuncInline.enumLetsAxis)).Length + 2]; // 스텝모터 5, 서보2 생성시 변수로 지정하면 0이 되어 상수로 입력
        ...
        public static double DefaultPCBWidth = 240; // 오토닉 스텝모터 폭 계산할 때 원점 기준 기본 PCB 폭
