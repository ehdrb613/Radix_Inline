    ...
    public partial class TestSite : Form
    {
        ...
        public void ApplyAllValue()
        {
            try
            {
                ...
                FuncInline.EnableSite(20, btnUseSite21.BackColor == Color.Lime);
                FuncInline.EnableSite(21, btnUseSite22.BackColor == Color.Lime);
                FuncInline.EnableSite(22, btnUseSite23.BackColor == Color.Lime);
                FuncInline.EnableSite(23, btnUseSite24.BackColor == Color.Lime);
                FuncInline.EnableSite(24, btnUseSite25.BackColor == Color.Lime);
                FuncInline.EnableSite(25, btnUseSite26.BackColor == Color.Lime);
        ...
        private void dataGridErrorCode_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (dataGridErrorCode.SelectedCells.Count > 0 && 
                    dataGridErrorCode.Rows[dataGridErrorCode.SelectedCells[0].RowIndex].Cells[0].Value != null)
        ...
        private void timerChange_Tick(object sender, EventArgs e)
        {
            ...
            {
                            btnUseSite1,  btnUseSite2,  btnUseSite3,  btnUseSite4,  btnUseSite5,  btnUseSite6,
                            btnUseSite7,  btnUseSite8,  btnUseSite9,  btnUseSite10, btnUseSite11, btnUseSite12,
                            btnUseSite13, btnUseSite14, btnUseSite15, btnUseSite16, btnUseSite17, btnUseSite18,
                            btnUseSite19, btnUseSite20, btnUseSite21, btnUseSite22, btnUseSite23, btnUseSite24,
                            btnUseSite25, btnUseSite26
            };
            // 세대별 사이트 수(MaxSiteCount)에 맞춰 안전하게 처리
            int count = Math.Min(pcbBtnsSite.Length, FuncInline.MaxSiteCount);
            for (int i = 0; i < count; i++)
            {
                int.TryParse(pcbBtnsSite[i].Name.Replace("btnUseSite", ""), out siteIndex);
                siteIndex--;
        ...
        private void TestSite_Shown(object sender, EventArgs e)
        {
            ...
            foreach (Control conSite in pnSiteUse.Controls) // 사이트 묶음 전체
            {
                if (conSite.GetType() == typeof(Panel))
                {
                    int siteNo = -1;
                    int.TryParse(conSite.Name.Replace("btnUseSite", ""), out siteNo);
                    if (siteNo > 0 && siteNo <= 26)
                    {
